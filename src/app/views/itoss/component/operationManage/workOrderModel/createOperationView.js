...
var createOperation = React.createClass({
    ...
    componentWillMount:function(){
      var workFlowTypes = this.props.workFlowTypes;//当前工单流程设计数据
      if(workFlowTypes.length <= 0){
        //获取当前工单流程设计数据
        this.props.dispatch(operationAction.get_workFlowTypes());
      };
      if(this.props.curWorkFlowId){
        //创建工单时，获取当前工单流程设计主表相关的模板数据
        var param ={
          workFlowId:this.props.curWorkFlowId,
          workWorderId:''
        };
        this.props.dispatch(operationAction.get_workOrderTemplatesFromWorkFlowId(param));
      };
    },
      ...
      return(
          <div id='createOperation' className='overviewDiv'>
            ...
            <OperationForm flowPersonnelListData={this.props.flowPersonnelListData} flowActionListData={this.props.flowActionListData}
                ...
                add_workOrderForm={(params, entity) =>dispatch(operationAction.add_workOrderForm(params, entity))}
                save_createOrderAndDetailTemplate={(data, detailData, entity) =>dispatch(operationAction.save_createOrderAndDetailTemplate(data, detailData, entity))}
