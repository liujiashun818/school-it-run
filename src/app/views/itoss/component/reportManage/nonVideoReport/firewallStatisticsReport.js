...
var FirewallStatisticsReport = React.createClass({
    render: function() {
        const { dispatch, firewallDailyData, firewallWeeklyData, firewallMonthlyData, firewallQuarterlyData, firewallYearlyData, firewallCustomData, curThreeNode, preThreeNode } = this.props;
        return (
            <div className='overviewDiv'>
                <NonVideoLeftList activeMenu={6} curThreeNode={curThreeNode} preThreeNode={preThreeNode} curName={this.props.curName} setCurName={data => dispatch(navbarActions.setCurName(data))} onGetCurThreeNode={curNode => dispatch(navbarActions.setCurThreeNode(curNode))} onGetPreThreeNode={curNode => dispatch(navbarActions.setPreThreeNode(curNode))}/>
                <FirewallStatisticsReportView firewallDailyData={firewallDailyData} firewallWeeklyData={firewallWeeklyData} firewallMonthlyData={firewallMonthlyData}
                    firewallQuarterlyData={firewallQuarterlyData} firewallYearlyData={firewallYearlyData} firewallCustomData={firewallCustomData}
...
function mapStateToProps(state) {
  const { firewallDailyData, firewallWeeklyData, firewallMonthlyData, firewallQuarterlyData, firewallYearlyData, firewallCustomData } = state.reportManageReducer
