...
var DatabaseStatisticsReport = React.createClass({
    render: function() {
        const { dispatch, databaseDailyData, databaseWeeklyData, databaseMonthlyData, databaseQuarterlyData, databaseYearlyData, databaseCustomData, curThreeNode, preThreeNode } = this.props;
        return (
            <div className='overviewDiv'>
                <NonVideoLeftList activeMenu={7} curThreeNode={curThreeNode} preThreeNode={preThreeNode} curName={this.props.curName} setCurName={data => dispatch(navbarActions.setCurName(data))} onGetCurThreeNode={curNode => dispatch(navbarActions.setCurThreeNode(curNode))} onGetPreThreeNode={curNode => dispatch(navbarActions.setPreThreeNode(curNode))}/>
                <DatabaseStatisticsReportView databaseDailyData={databaseDailyData} databaseWeeklyData={databaseWeeklyData} databaseMonthlyData={databaseMonthlyData}
                    databaseQuarterlyData={databaseQuarterlyData} databaseYearlyData={databaseYearlyData} databaseCustomData={databaseCustomData}
...
function mapStateToProps(state) {
  const { databaseDailyData, databaseWeeklyData, databaseMonthlyData, databaseQuarterlyData, databaseYearlyData, databaseCustomData } = state.reportManageReducer
