...
var DVRStatisticsReport = React.createClass({
    render: function() {
        const { dispatch, dvrStorageDailyData, dvrStorageWeeklyData, dvrStorageMonthlyData, dvrStorageQuarterlyData, dvrStorageYearlyData, dvrStorageCustomData, dvrStorageCustomLineData, curThreeNode, preThreeNode } = this.props;
        return (
            <div className='overviewDiv'>
                <NonVideoLeftList activeMenu={1} curName={this.props.curName} curThreeNode={curThreeNode} preThreeNode={preThreeNode} setCurName={data => dispatch(navbarActions.setCurName(data))} onGetCurThreeNode={curNode => dispatch(navbarActions.setCurThreeNode(curNode))} onGetPreThreeNode={curNode => dispatch(navbarActions.setPreThreeNode(curNode))}/>
                <DVRStatisticsReportView dvrStorageDailyData={dvrStorageDailyData} dvrStorageWeeklyData={dvrStorageWeeklyData} dvrStorageMonthlyData={dvrStorageMonthlyData}
                    dvrStorageQuarterlyData={dvrStorageQuarterlyData} dvrStorageYearlyData={dvrStorageYearlyData} dvrStorageCustomData={dvrStorageCustomData} dvrStorageCustomLineData={dvrStorageCustomLineData}
...
function mapStateToProps(state) {
  const { dvrStorageDailyData, dvrStorageWeeklyData, dvrStorageMonthlyData, dvrStorageQuarterlyData, dvrStorageYearlyData, dvrStorageCustomData, dvrStorageCustomLineData } = state.reportManageReducer
