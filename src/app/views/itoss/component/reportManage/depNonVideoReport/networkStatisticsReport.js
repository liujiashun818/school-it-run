...
var NetworkStatisticsReport = React.createClass({
    render: function() {
        const { dispatch, networkDailyData, networkWeeklyData, networkMonthlyData, networkQuarterlyData, networkYearlyData, networkCustomData, networkCustomLineData, curThreeNode, preThreeNode } = this.props;
        return (
            <div className='overviewDiv'>
                <NonVideoLeftList activeMenu={5} curName={this.props.curName} setCurName={data => dispatch(navbarActions.setCurName(data))} curThreeNode={curThreeNode} preThreeNode={preThreeNode} onGetCurThreeNode={curNode => dispatch(navbarActions.setCurThreeNode(curNode))} onGetPreThreeNode={curNode => dispatch(navbarActions.setPreThreeNode(curNode))}/>
                <NetworkStatisticsReportView networkDailyData={networkDailyData} networkWeeklyData={networkWeeklyData} networkMonthlyData={networkMonthlyData}
                    networkQuarterlyData={networkQuarterlyData} networkYearlyData={networkYearlyData} networkCustomData={networkCustomData} networkCustomLineData={networkCustomLineData}
...
function mapStateToProps(state) {
  const { networkDailyData, networkWeeklyData, networkMonthlyData, networkQuarterlyData, networkYearlyData, networkCustomData, networkCustomLineData } = state.reportManageReducer
