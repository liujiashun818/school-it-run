...
var ServerStatisticsReport = React.createClass({
    render: function() {
        const { dispatch, serverDailyData, serverWeeklyData, serverMonthlyData, serverQuarterlyData, serverYearlyData, serverCustomData, serverCustomLineData, curThreeNode, preThreeNode } = this.props;
        return (
            <div className='overviewDiv'>
                <NonVideoLeftList activeMenu={4} curName={this.props.curName} setCurName={data => dispatch(navbarActions.setCurName(data))} curThreeNode={curThreeNode} preThreeNode={preThreeNode} onGetCurThreeNode={curNode => dispatch(navbarActions.setCurThreeNode(curNode))} onGetPreThreeNode={curNode => dispatch(navbarActions.setPreThreeNode(curNode))}/>
                <ServerStatisticsReportView serverDailyData={serverDailyData} serverWeeklyData={serverWeeklyData} serverMonthlyData={serverMonthlyData}
                    serverQuarterlyData={serverQuarterlyData} serverYearlyData={serverYearlyData} serverCustomData={serverCustomData} serverCustomLineData={serverCustomLineData}
...
function mapStateToProps(state) {
  const { serverDailyData, serverWeeklyData, serverMonthlyData, serverQuarterlyData, serverYearlyData, serverCustomData, serverCustomLineData } = state.reportManageReducer
