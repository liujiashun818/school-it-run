/**
* Created by yinxuexue  2016/03/15.
* 资产维修报表
*/
...
var AssetMaintainViewTable = require('./assetMaintainViewTable').AssetMaintainViewTable;
var Util = require('../../../../../utils/util');
var DateChange = require('../../../../../utils/dateChange.js');
var curAssetMaintaintabsState = 3;

var AssetMaintainView = React.createClass({
    mixins: [History],
    /*getStateFromFlux: function() {
        var flux = this.getFlux();
        return {
            itoss:flux.store("AssetReportStore").getState()
        }
    },*/
    ...
    componentDidMount: function() {
      curAssetMaintaintabsState = 3;
      //清空报表数据
      this.props.set_InitAssetReportData();
      if (this.isMounted()) {
        if(document.getElementById('maintainReportDesViewDiv') != null) {
            document.getElementById('maintainReportDesViewDiv').style.height = $(window).height() - 110 - 30 + 'px';
        }
    ...
    _getCheckData:function(param){
      var key = param.key;
      var tabState = curAssetMaintaintabsState;
      var name = "";
      var data = {};
      ...
      switch (key) {
        case 3:
            ...
            this.props.get_maintain_report_data({
                data: data,
    ...
    handleTabClick:function(key){
      ...
      curAssetMaintaintabsState = key;
      var timeValue = "";
      if (key == 3) {
        var timeValue = ReactDOM.findDOMNode(this.refs.monthReportTime).firstChild.value;
        if(timeValue == ""){
          this._getInitMonthlyData();
        }else{
          this.setState({initdate: timeValue});
        ...
        var timeValue = ReactDOM.findDOMNode(this.refs.yearReportTime).firstChild.value;
        if(timeValue == ""){
    ...
    render: function() {
        return (
            <div id="maintainReportDesViewDiv" className="overviewDesViewDiv assetDesViewDiv">
                        ...
                        <fieldset className="maintainOrderTable assetManageTable">
                            <div className="contentDiv tab-content marginleft_none">
                                <div className="tab-pane active" id="tab_2">
                                    ...
                                    <div className="staticChartDiv col-md-12">
                                        <div className="re-table">
                                          <AssetMaintainViewTable report={this.props.report} tableId="reportMonthlyReportTable" chartKey={this.state.tabsState} />
                                        </div>
                                    </div>
                                </div>
                                <div className="tab-pane" id="tab_4">
                                    ...
                                    <div className="staticChartDiv col-md-12">
                                      <div className="re-table">
                                        <AssetMaintainViewTable report={this.props.report} tableId="reportYearlyReportTable" chartKey={this.state.tabsState} />
                                      </div>
                                    </div>
                                </div>
                            </div>
...
module.exports = AssetMaintainView;
