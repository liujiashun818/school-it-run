 ...
 * 处理器 未完成 modified by Fenzhou on 2016/06/22.
 */
require('bootstrap');
...
var CreateMonitorView_StorageProcessors = React.createClass({
    mixins: [History],
    ...
    componentDidMount: function() {
        if(document.getElementById('createMonitorView_StorageProcessors') != null) {
            document.getElementById('createMonitorView_StorageProcessors').style.height = $(window).height() - 110 - 30 + 'px';
        };
        if (this.isMounted()) {
          ...
          if(monitorsPropertyEdit){
            ...
            if(monitorsPropertyData){
              ...
              $("#Title_StorageProcessors").val(Title); //标题
              var IsRunningAlarm = monitorsPropertyData.IsRunningAlarm;
              if (IsRunningAlarm) {
                //运用到告警
                // document.getElementById("IsRunningAlarm_StorageProcessors").checked=true;
                $("#IsRunningAlarm_StorageProcessors").attr("checked", 'true');
              };
              var RefreshTime = monitorsPropertyData.RefreshTime;
              $("#RefreshTime_StorageProcessors").val(RefreshTime); //刷新频率
              ...
              if (IsErrorChecking) {
                $("#IsErrorChecking_StorageProcessors").attr("checked", 'true'); //监测器错误校验
              }
              var ErrorCheckingTime = monitorsPropertyData.ErrorCheckingTime;
              $("#ErrorCheckingTime_StorageProcessors").val(ErrorCheckingTime); //错误频率
              ...
              if (IsDisabled) {
                $("#IsDisabled_StorageProcessors").attr("checked", 'true'); //禁用监测器
              };
              ...
              $("#description_StorageProcessors").val(Description); //描述
            };
          }else{
              //增加监测器
              $("#IsRunningAlarm_StorageProcessors").attr("checked",true);
              $("#RefreshTime_StorageProcessors").val("10");
              $("#Title_StorageProcessors").val("");
              $("#IsErrorChecking_StorageProcessors").attr("checked",false);
              $("#ErrorCheckingTime_StorageProcessors").val("0");
              $("#IsDisabled_StorageProcessors").attr("checked",false);
              $("#description_StorageProcessors").val("");
          }
    ...
    handleOnClickSave:function(){
      ...
      //保存监测器数据
      //编辑监测器属性-保存
      var Title = $.trim($("#Title_StorageProcessors").val());//标题
      if(!Title){
      ...
      //运用到告警
      if(document.getElementById("IsRunningAlarm_StorageProcessors").checked){
        IsRunningAlarm = true;
      };
      var RefreshTime = $.trim($("#RefreshTime_StorageProcessors").val()); //刷新频率
      if(!RefreshTime){
      ...
      if (document.getElementById("IsErrorChecking_StorageProcessors").checked) {
        IsErrorChecking = true;
      };
      var ErrorCheckingTime = $.trim($("#ErrorCheckingTime_StorageProcessors").val()); //错误频率;
      ...
      if (document.getElementById("IsDisabled_StorageProcessors").checked) {
        IsDisabled = true; //禁用监测器
      };
      var TaskScheduler = this.state.taskScheduler;//任务计划
      var Description = $.trim($("#description_StorageProcessors").val()); //描述
      ...
      if(monitorsPropertyEdit){
        ...
        var propertyData = {
          ...
          TaskScheduler: TaskScheduler,
          Description: Description,
          MonitorPackage: "com.siteview.monitor.StorageProcessorsMonitor",
          GroupId: GroupId,
    ...
    render: function() {
        ...
        return (
            <div id="createMonitorView_StorageProcessors" className="overviewDesViewDiv monitoringSettingTab operationButtons">
                      ...
                      <table>
                        <tbody>
                          <tr>
                            ...
                            <td className="paddingleft10" style={{width:"15%"}}>处理器<span style={{color: "red"}}>*</span></td>
                          ...
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="监测器生成事件后告警">
                                    <input id="IsRunningAlarm_StorageProcessors" type="checkbox" className="input-checkbox"/>运用到告警
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>刷新频率<span style={{color: "red"}}>*</span></td>
                            <td style={{width:"25%"}}><input id="RefreshTime_StorageProcessors" type="number" title="监测器的监测频率"/></td>
                            <td style={{width:"5%"}}>
                              <ReactWidgets.DropdownList id="RefreshUnit_StorageProcessors" data={timeUnitData} value={this.state.refreshUnit} onChange={this.handleOnChangeRefreshUnit}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>标题<span style={{color: "red"}}>*</span></td>
                            <td colSpan="5" style={{width:"75%"}}><input id="Title_StorageProcessors" type="text" title="监测器显示的名称"/></td>
                          </tr>
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="如果监测器检测到错误，立即再次执行监测器校验错误">
                                    <input id="IsErrorChecking_StorageProcessors" type="checkbox" className="input-checkbox"/>监测器错误校验
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>错误频率</td>
                            <td style={{width:"25%"}}><input id="ErrorCheckingTime_StorageProcessors" type="number" title="处于“错误”条件时监测器刷新频率"/></td>
                            <td style={{width:"5%"}}>
                              <ReactWidgets.DropdownList id="ErrorCheckingUnit_StorageProcessors" data={timeUnitData} value={this.state.errorCheckingUnit} onChange={this.handleOnChangeErrorCheckingUnit}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>禁用</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="勾选此项后监测器停止监测">
                                    <input type="checkbox" id="IsDisabled_StorageProcessors" className="input-checkbox"/>禁用监测器
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>任务计划</td>
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList id="taskScheduler_StorageProcessors" data={taskSchedulerData} value={this.state.taskScheduler} onChange={this.handleOnChangeTaskScheduler} title="从下拉框中选择监测器执行何种任务计划"/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>描述</td>
                            <td colSpan="5" style={{width:"75%"}}>
                                <textarea id="description_StorageProcessors" className="form-control" rows="5"></textarea>
                            </td>
                          </tr>
...
// module.exports = CreateMonitorView_Status;
CreateMonitorView_StorageProcessors.propTypes = {
...
export default connect(mapStateToProps)(CreateMonitorView_StorageProcessors)
