...
var facilityData = ["Kernel","Cron","Security","FTP Daemon","NTP","Log audit","Log aler","Clock Daemon","Local0","Local1","Local2","Local3","Local4","Local5","Local6","Local7","User","Mail","Daemon","Auth","Syslog","Lpr","News","UUCP"];
var levelData = ["Error","Emergency","Alert","Critical","Warning","Notice","Informational","Debug"];

var matchingRangeData = [
  {id:'1DAY', name:'1天'},
  {id:'3DAY', name:'3天'},
  {id:'5DAY', name:'5天'},
  {id:'1WEEK', name:'1周'},
...
var CreateMonitorView_SysLog = React.createClass({
    mixins: [History],
    getInitialState: function() {
        return {
            ...
            matchingRange:matchingRangeData[0]
        }
    },
    componentDidMount: function() {
        if(document.getElementById('createMonitorView_SysLog') != null) {
            document.getElementById('createMonitorView_SysLog').style.height = $(window).height() - 110 - 30 + 'px';
        };
        if (this.isMounted()) {
          ...
          if(monitorsPropertyEdit){
            ...
            if(monitorsPropertyData){
              ...
              var SpecialProperties = SpecialPropertiesTemp.split(","); //ping 的自有属性 SpecialProperties:facility=FTP Daemon,level=Critical,matchPeriod=3DAY,hostName=192.168.9.147,matchValue=message%
              for (var i = 0; i < SpecialProperties.length; i++) {
                if(SpecialProperties[i].indexOf("facility") >= 0){
                  //匹配范围
                  var temp = SpecialProperties[i].split("=");
                  if (temp.length == 2) {
                    if (temp[1]) {
                      //$("#facility_SysLog").val(temp[1]);
                      this.setState({facility: temp[1]});
                    }
                  }
                } else if(SpecialProperties[i].indexOf("level") >= 0) {
                  //匹配范围
                  var temp = SpecialProperties[i].split("=");
                  if (temp.length == 2) {
                    if (temp[1]) {
                      //$("#level_SysLog").val(temp[1]);
                      this.setState({level: temp[1]});
                    }
                  }
                } else if (SpecialProperties[i].indexOf("matchPeriod") >= 0) {
                  //匹配范围
                  var temp = SpecialProperties[i].split("=");
                  if (temp.length == 2) {
                    if (temp[1]) {
                      //$("#matchPeriod_SysLog").val(temp[1]);
                      for (var n = 0; n < matchingRangeData.length; n++) {
                        if(matchingRangeData[n].id == temp[1]){
                          this.setState({matchingRange: matchingRangeData[n]});
                          break;
                        }
                      }
                    }
                  }
                } else if (SpecialProperties[i].indexOf("matchValue") >= 0) {
                  var temp = SpecialProperties[i].split("=");
                  if (temp.length == 2) {
                    if (temp[1]) {
                      $("#matchValue_SysLog").val(temp[1]);//匹配内容
                    }
                  }
                  ...
                  if (temp.length == 2) {
                    if (temp[1]) {
                      $("#hostName_SysLog").val(temp[1]); //IP
                    }
                  }
                }
              };
              var Title = monitorsPropertyData.Title;
              $("#Title_SysLog").val(Title); //标题
              var IsRunningAlarm = monitorsPropertyData.IsRunningAlarm;
              if (IsRunningAlarm) {
                //运用到告警
                // document.getElementById("IsRunningAlarm_SysLog").checked=true;
                $("#IsRunningAlarm_SysLog").attr("checked", 'true');
              };
              var RefreshTime = monitorsPropertyData.RefreshTime;
              $("#RefreshTime_SysLog").val(RefreshTime); //刷新频率
              ...
              if (IsErrorChecking) {
                $("#IsErrorChecking_SysLog").attr("checked", 'true'); //监测器错误校验
              };
              var ErrorCheckingTime = monitorsPropertyData.ErrorCheckingTime;
              $("#ErrorCheckingTime_SysLog").val(ErrorCheckingTime); //错误频率
              ...
              if (IsDisabled) {
                $("#IsDisabled_SysLog").attr("checked", 'true'); //禁用监测器
              };
              ...
              $("#description_SysLog").val(Description); //描述
            };
          }else{
              //增加监测器
              $("#Title_SysLog").val("SysLog");//标题
              $("#IsRunningAlarm_SysLog").attr("checked",true);
              $("#RefreshTime_SysLog").val("10");
              $("#IsErrorChecking_SysLog").attr("checked",false);
              $("#ErrorCheckingTime_SysLog").val("10");
              $("#IsDisabled_SysLog").attr("checked",false);
              $("#description_SysLog").val("");
          }
        };
    },
    handleOnClickSave:function(){
      ...
      //保存监测器数据
      //编辑监测器属性-保存
      var specialProperties = ""; //的自有属性 SpecialProperties:facility=FTP Daemon,level=Critical,matchPeriod=3DAY,hostName=192.168.9.147,matchValue=message%
      var facility = this.state.facility;
      var level = this.state.level;
      var matchingRange = this.state.matchingRange;//匹配范围
      var matchingContent = $.trim($("#matchValue_SysLog").val());//匹配内容
      var hostname = $.trim($("#hostName_SysLog").val()); //IP地址
      specialProperties = "facility=" + facility + ",level=" + level;
      if(matchingRange){
        if(specialProperties){
          specialProperties = specialProperties + ",matchPeriod=" + matchingRange.id;
        }else{
          specialProperties = "matchPeriod=" + matchingRange.id;
        }
      };
      if(hostname) {
        if(specialProperties){
      ...
      if(matchingContent) {
        if(specialProperties){
          specialProperties = specialProperties + ",matchValue=" + matchingContent;
        }else{
          specialProperties =  "matchValue="+ matchingContent;
        }
      };
      var Title = $.trim($("#Title_SysLog").val());//标题
      if(!Title){
      ...
      //运用到告警
      if(document.getElementById("IsRunningAlarm_SysLog").checked){
        IsRunningAlarm = true;
      };
      var RefreshTime = $.trim($("#RefreshTime_SysLog").val()); //刷新频率
      if(!RefreshTime){
      ...
      if (document.getElementById("IsErrorChecking_SysLog").checked) {
        IsErrorChecking = true;
      };
      var ErrorCheckingTime = $.trim($("#ErrorCheckingTime_SysLog").val()); //错误频率;
      ...
      if (document.getElementById("IsDisabled_SysLog").checked) {
        IsDisabled = true; //禁用监测器
      };
      var TaskScheduler = this.state.taskScheduler;//任务计划
      var Description = $.trim($("#description_SysLog").val()); //描述
      ...
      if(monitorsPropertyEdit){
        ...
        var propertyData = {
          ...
          TaskScheduler: TaskScheduler,
          Description: Description,
          MonitorPackage: "com.siteview.monitor.SysLogMonitor",
          GroupId: GroupId,
    ...
    handleOnChangeFacility: function(e) {
        this.setState({facility: e});
    },
    handleOnChangeLevel: function(e) {
    ...
    handleOnChangeMatchingRange:function(e){
      this.setState({matchingRange: e});
    },
    render: function() {
        ...
        return (
            <div id="createMonitorView_SysLog" className="overviewDesViewDiv monitoringSettingTab operationButtons">
                      ...
                      <table>
                        <tbody>
                          <tr>
                            ...
                            <td className="paddingleft10" style={{width:"15%"}}>IP</td>
                            <td colSpan="2" style={{width:"30%"}}><input id="hostName_SysLog" type="text" title="产生日志的设备IP，如果匹配多个IP，IP之间用；分隔如192.168.5.1;192.168.5.2不输入任何值匹配所有设备。"/></td>
                            <td className="paddingleft10" style={{width:"15%"}}>Facility</td>
                            <td colSpan="2" style={{width:"30%"}}>
                              <ReactWidgets.DropdownList id="facility_SysLog" data={facilityData} value={this.state.facility} onChange={this.handleOnChangeFacility} title="匹配大于等于该级别的事件。"/>
                            </td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>Level</td>
                            <td colSpan="2" style={{width:"30%"}}>
                              <ReactWidgets.DropdownList id="level_SysLog" className="dataDictDropDown" data={levelData} value={this.state.level} onChange={this.handleOnChangeLevel} title="匹配大于等于该级别的事件。"/>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>运用到告警</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="监测器生成事件后告警。">
                                    <input id="IsRunningAlarm_SysLog" type="checkbox" className="input-checkbox"/>运用到告警
                                </label>
                            </td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>刷新频率<span style={{color: "red"}}>*</span></td>
                            <td style={{width:"25%"}}><input id="RefreshTime_SysLog" type="number" title="监测器的监测频率。"/></td>
                            <td style={{width:"5%"}}>
                              <ReactWidgets.DropdownList className="dataDictDropDown" id="RefreshUnit_SysLog" data={timeUnitData} value={this.state.refreshUnit} onChange={this.handleOnChangeRefreshUnit}/>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>标题<span style={{color: "red"}}>*</span></td>
                            <td colSpan="2" style={{width:"30%"}}><input id="Title_SysLog" type="text" title="监测器显示的名称。"/></td>
                          </tr>
                          <tr>
                            <th rowSpan="4" style={{width:"10%"}}>高级选项</th>
                            <td className="paddingleft10" style={{width:"15%"}}>匹配范围</td>
                            <td colSpan="2" style={{width:"30%"}}>
                              <ReactWidgets.DropdownList className="dataDictDropDown"  id="matchPeriod_SysLog" data={matchingRangeData} value={this.state.matchingRange} textField='name' onChange={this.handleOnChangeMatchingRange} title="匹配的时间范围。"/>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>匹配内容</td>
                            <td colSpan="2" style={{width:"30%"}}><input id="matchValue_SysLog" type="text" title="需要匹配的内容，如：前匹配message%，后匹配%message，全匹配message，包含匹配%message%。"/></td>
                          </tr>
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="如果监测器检测到错误，立即再次执行监测器校验错误。">
                                    <input id="IsErrorChecking_SysLog" type="checkbox" className="input-checkbox"/>监测器错误校验
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>错误频率</td>
                            <td style={{width:"25%"}}><input id="ErrorCheckingTime_SysLog" type="number" title="处于“错误”条件时监测器刷新频率。"/></td>
                            <td style={{width:"5%"}}>
                              <ReactWidgets.DropdownList className="dataDictDropDown" id="ErrorCheckingUnit_SysLog" data={timeUnitData} value={this.state.errorCheckingUnit} onChange={this.handleOnChangeErrorCheckingUnit}/>
                            </td>
                          </tr>
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="勾选此项后监测器停止监测。">
                                    <input id="IsDisabled_SysLog" type="checkbox" className="input-checkbox"/>禁用监测器
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>任务计划</td>
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList className="dataDictDropDown"  id="taskScheduler_SysLog" data={taskSchedulerData} value={this.state.taskScheduler} onChange={this.handleOnChangeTaskScheduler} title="从下拉框中选择监测器执行何种任务计划。"/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>描述</td>
                            <td colSpan="5" style={{width:"75%"}}>
                                <textarea id="description_SysLog" className="form-control" rows="5"></textarea>
                            </td>
                          </tr>
...
// module.exports = CreateMonitorView_SysLog;
CreateMonitorView_SysLog.propTypes = {
...
export default connect(mapStateToProps)(CreateMonitorView_SysLog)
