...
var CreateMonitorView_DataQuery = React.createClass({
    mixins: [History],
    ...
    componentDidMount: function() {
        if(document.getElementById('createMonitorView_DataQuery') != null) {
            document.getElementById('createMonitorView_DataQuery').style.height = $(window).height() - 110 - 30 + 'px';
        };
        if (this.isMounted()) {
          ...
          if(monitorsPropertyEdit){
            ...
            if(monitorsPropertyData){
              ...
              var SpecialProperties = SpecialPropertiesTemp.split(","); //ping 的自有属性SpecialProperties:"sql_query=select * from Monitor,file_sql_path=D:\gitwork"
              for (var i = 0; i < SpecialProperties.length; i++) {
                if (SpecialProperties[i].indexOf("sql_query") >= 0) {
                  //SQL语句
                  var temp = SpecialProperties[i].split("=");
                  if (temp.length == 2) {
                    if (temp[1]) {
                      $("#sql_query_DataQuery").val(temp[1]);
                    }
                  }
                } else if (SpecialProperties[i].indexOf("file_sql_path") >= 0) {
                  //文件路径
                  var temp = SpecialProperties[i].split("=");
                  if (temp.length == 2) {
                    if (temp[1]) {
                      $("#file_sql_path_DataQuery").val(temp[1]);
                    }
                  }
                }
              };
              var Title = monitorsPropertyData.Title;
              $("#Title_DataQuery").val(Title); //标题
              var IsRunningAlarm = monitorsPropertyData.IsRunningAlarm;
              if (IsRunningAlarm) {
                //运用到告警
                // document.getElementById("IsRunningAlarm_DataQuery").checked=true;
                $("#IsRunningAlarm_DataQuery").attr("checked", 'true');
              };
              var RefreshTime = monitorsPropertyData.RefreshTime;
              $("#RefreshTime_DataQuery").val(RefreshTime); //刷新频率
              ...
              if (IsErrorChecking) {
                $("#IsErrorChecking_DataQuery").attr("checked", 'true'); //监测器错误校验
              }
              var ErrorCheckingTime = monitorsPropertyData.ErrorCheckingTime;
              $("#ErrorCheckingTime_DataQuery").val(ErrorCheckingTime); //错误频率
              ...
              if (IsDisabled) {
                $("#IsDisabled_DataQuery").attr("checked", 'true'); //禁用监测器
              };
              ...
              $("#description_DataQuery").val(Description); //描述
            };
          }else{
              //增加监测器
              $("#Title_DataQuery").val("数据库查询");
              $("#IsRunningAlarm_DataQuery").attr("checked",true);
              $("#RefreshTime_DataQuery").val("10");
              $("#IsErrorChecking_DataQuery").attr("checked",false);
              $("#ErrorCheckingTime_DataQuery").val("10");
              $("#IsDisabled_DataQuery").attr("checked",false);
              $("#description_DataQuery").val("");
          }
        };
    },
    handleOnClickSave:function(){
      ...
      //保存监测器数据
      //编辑监测器属性-保存
      var specialProperties = ""; //ping 的自有属性 SpecialProperties: "sql_query=select * from Monitor,file_sql_path=D:\gitwork"
      var sql_query = $.trim($("#sql_query_DataQuery").val());//SQL语句
      var file_sql_path = $.trim($("#file_sql_path_DataQuery").val());//文件路径
      if(sql_query){
        specialProperties = "sql_query=" + sql_query;
      };
      if(file_sql_path) {
        if(specialProperties){
          specialProperties = specialProperties + ",file_sql_path=" + file_sql_path;
        }else{
          specialProperties =  "file_sql_path="+ file_sql_path;
        }
      };

      var Title = $.trim($("#Title_DataQuery").val());//标题
      if(!Title){
      ...
      //运用到告警
      if(document.getElementById("IsRunningAlarm_DataQuery").checked){
        IsRunningAlarm = true;
      };
      var RefreshTime = $.trim($("#RefreshTime_DataQuery").val()); //刷新频率
      if(!RefreshTime){
      ...
      if (document.getElementById("IsErrorChecking_DataQuery").checked) {
        IsErrorChecking = true;
      };
      var ErrorCheckingTime = $.trim($("#ErrorCheckingTime_DataQuery").val()); //错误频率;
      ...
      if (document.getElementById("IsDisabled_DataQuery").checked) {
        IsDisabled = true; //禁用监测器
      };
      var TaskScheduler = this.state.taskScheduler;//任务计划
      var Description = $.trim($("#description_DataQuery").val()); //描述
      ...
      if(monitorsPropertyEdit){
        ...
        var propertyData = {
          ...
          TaskScheduler: TaskScheduler,
          Description: Description,
          MonitorPackage: "com.siteview.monitor.dataquery.DataQueryMonitor",
          GroupId: GroupId,
    ...
    render: function() {
        ...
        return (
            <div id="createMonitorView_DataQuery" className="overviewDesViewDiv monitoringSettingTab operationButtons">
                      ...
                      <table>
                        <tbody>
                          <tr>
                            ...
                            <td className="paddingleft10" style={{width:"15%"}}>SQL语句</td>
                            <td colSpan="2" style={{width:"30%"}}><input id="sql_query_DataQuery" type="text"/></td>
                            <td className="paddingleft10" style={{width:"15%"}}>文件路径</td>
                            <td colSpan="2" style={{width:"30%"}}><input id="file_sql_path_DataQuery" type="text"/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>运用到告警</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="监测器生成事件后告警">
                                    <input id="IsRunningAlarm_DataQuery" type="checkbox" className="input-checkbox"/>运用到告警
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>刷新频率<span style={{color: "red"}}>*</span></td>
                            <td style={{width:"25%"}}><input id="RefreshTime_DataQuery" type="number" title="监测器的监测频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="RefreshUnit_DataQuery" data={timeUnitData} value={this.state.refreshUnit} onChange={this.handleOnChangeRefreshUnit}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>标题<span style={{color: "red"}}>*</span></td>
                            <td colSpan="5" style={{width:"75%"}}><input id="Title_DataQuery" type="text" title="监测器显示的名称"/></td>
                          </tr>
                          ...
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="如果监测器检测到错误，立即再次执行监测器校验错误">
                                    <input id="IsErrorChecking_DataQuery" type="checkbox" className="input-checkbox"/>监测器错误校验
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>错误频率</td>
                            <td style={{width:"25%"}}><input id="ErrorCheckingTime_DataQuery" type="number" title="处于“错误”条件时监测器刷新频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="ErrorCheckingUnit_DataQuery" data={timeUnitData} value={this.state.errorCheckingUnit} onChange={this.handleOnChangeErrorCheckingUnit}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>禁用</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="勾选此项后监测器停止监测">
                                    <input id="IsDisabled_DataQuery" type="checkbox" className="input-checkbox"/>禁用监测器
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>任务计划</td>
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList  id="taskScheduler_DataQuery" data={taskSchedulerData} value={this.state.taskScheduler} onChange={this.handleOnChangeTaskScheduler} title="从下拉框中选择监测器执行何种任务计划"/></td>
                          </tr>
                          <tr>
                              <td className="paddingleft10" style={{width:"15%"}}>描述</td>
                              <td colSpan="5" style={{width:"75%"}}><textarea id="description_DataQuery" className="form-control" style={{height:"60px"}}/></td>
                          </tr>
...
// module.exports = CreateMonitorView_DataQuery;
CreateMonitorView_DataQuery.propTypes = {
...
export default connect(mapStateToProps)(CreateMonitorView_DataQuery)
