...
var CreateMonitorView_SnmpGigabitInterface = React.createClass({
    mixins: [History],
    ...
    componentDidMount: function() {
        if(document.getElementById('createMonitorView_SnmpGigabitInterface') != null) {
            document.getElementById('createMonitorView_SnmpGigabitInterface').style.height = $(window).height() - 110 - 30 + 'px';
        }
        if (this.isMounted()) {
            ...
            if(monitorsPropertyData){
              ...
              for (var i = 0; i < SpecialProperties.length; i++) {
                if (SpecialProperties[i].indexOf("networkName") >= 0) {
                  ...
                  if (temp.length == 2) {
                    if (temp[1]) {
                    //   $("#networkName_SnmpGigabitInterface").val(temp[1]);
                      networkName = temp[1];
                    }
                  }
              ...
              $("#Title_SnmpGigabitInterface").val(Title); //标题
              var IsRunningAlarm = monitorsPropertyData.IsRunningAlarm;
              if (IsRunningAlarm) {
                //运用到告警
                $("#IsRunningAlarm_SnmpGigabitInterface").attr("checked", 'true');
              };
              var RefreshTime = monitorsPropertyData.RefreshTime;
              $("#RefreshTime_SnmpGigabitInterface").val(RefreshTime); //刷新频率
              ...
              if (IsErrorChecking) {
                $("#IsErrorChecking_SnmpGigabitInterface").attr("checked", 'true'); //监测器错误校验
              }
              var ErrorCheckingTime = monitorsPropertyData.ErrorCheckingTime;
              $("#ErrorCheckingTime_SnmpGigabitInterface").val(ErrorCheckingTime); //错误频率
              ...
              if (IsDisabled) {
                $("#IsDisabled_SnmpGigabitInterface").attr("checked", 'true'); //禁用监测器
              };
              ...
              $("#description_SnmpGigabitInterface").val(Description); //描述
            };
          }else{
              //增加监测器
              $("#networkName_SnmpGigabitInterface").val("");
              $("#IsRunningAlarm_SnmpGigabitInterface").attr("checked",true);
              $("#RefreshTime_SnmpGigabitInterface").val("10");
              $("#Title_SnmpGigabitInterface").val("网络接口_千兆");
              $("#IsErrorChecking_SnmpGigabitInterface").attr("checked",false);
              $("#ErrorCheckingTime_SnmpGigabitInterface").val("10");
              $("#IsDisabled_SnmpGigabitInterface").attr("checked",false);
              $("#description_SnmpGigabitInterface").val("");
          }
    ...
    shouldComponentUpdate: function(nextProps, nextState) {
        if (!bUpdateInDidMount && nextProps.selectedMonitorNetwork !== this.props.selectedMonitorNetwork) {
            ...
            var Title = "千兆网络接口:"+ networkName;
            $("#Title_SnmpGigabitInterface").val(Title);
        }
    ...
    handleOnClickSave:function(){
      ...
      var networkName = $.trim($("#networkName_SnmpGigabitInterface").val());
      if(!networkName){
      ...
      var Title = $.trim($("#Title_SnmpGigabitInterface").val());//标题
      if(!Title){
      ...
      //运用到告警
      if(document.getElementById("IsRunningAlarm_SnmpGigabitInterface").checked){
        IsRunningAlarm = true;
      };
      var RefreshTime = $.trim($("#RefreshTime_SnmpGigabitInterface").val()); //刷新频率
      if(!RefreshTime){
      ...
      if (document.getElementById("IsErrorChecking_SnmpGigabitInterface").checked) {
        IsErrorChecking = true;
      };
      var ErrorCheckingTime = $.trim($("#ErrorCheckingTime_SnmpGigabitInterface").val()); //错误频率;
      ...
      if (document.getElementById("IsDisabled_SnmpGigabitInterface").checked) {
        IsDisabled = true; //禁用监测器
      };
      var TaskScheduler = this.state.taskScheduler;//任务计划
      var Description = $.trim($("#description_SnmpGigabitInterface").val()); //描述
      ...
      if(monitorsPropertyEdit){
        ...
        var propertyData = {
          RecId: recId,
          MonitorType: "SnmpGigabitInterface",
        ...
        var propertyData = {
          ...
          TaskScheduler: TaskScheduler,
          Description: Description,
          MonitorPackage: "com.siteview.monitor.SnmpGigabitInterfaceMonitor",
          GroupId: GroupId,
    ...
    render: function() {
        ...
        return (
            <div id="createMonitorView_SnmpGigabitInterface" className="overviewDesViewDiv monitoringSettingTab operationButtons">
                      ...
                      <table>
                        <tbody>
                          <tr>
                            ...
                            <td style={{width:"25%"}}><input id="networkName_SnmpGigabitInterface" type="text" value={selectedMonitorNetwork?selectedMonitorNetwork.name:""}/></td>
                            <td style={{width:"5%"}}><button type="button" className="btn btn-default btnGetModel" style={{width:"100%"}} data-toggle="modal" data-target="#monitorGigabitInterfaceTableModal" onClick={this.handleOnClickGetMonitorNetwork}>获取接口列表</button></td>
                            <td className="paddingleft10" style={{width:"15%"}}>运用到告警</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="监测器生成事件后告警">
                                    <input id="IsRunningAlarm_SnmpGigabitInterface" type="checkbox" className="input-checkbox"/>运用到告警
                                </label>
                            </td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>刷新频率<span style={{color: "red"}}>*</span></td>
                            <td style={{width:"25%"}}><input id="RefreshTime_SnmpGigabitInterface" type="number" title="监测器的监测频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="RefreshUnit_SnmpGigabitInterface" data={timeUnitData} value={this.state.refreshUnit} onChange={this.handleOnChangeRefreshUnit}/></td>
                            <td className="paddingleft10" style={{width:"15%"}}>标题<span style={{color: "red"}}>*</span></td>
                            <td colSpan="2" style={{width:"30%"}}><input id="Title_SnmpGigabitInterface" type="text" title="监测器的显示名称"/></td>
                          </tr>
                          ...
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="如果监测器检测到错误，立即再次执行监测器以校验错误">
                                    <input id="IsErrorChecking_SnmpGigabitInterface" type="checkbox" className="input-checkbox"/>监测器错误校验
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>错误频率</td>
                            <td style={{width:"25%"}}><input id="ErrorCheckingTime_SnmpGigabitInterface" type="number" title="处于“错误”条件时监测器的刷新频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="ErrorCheckingUnit_SnmpGigabitInterface" data={timeUnitData} value={this.state.errorCheckingUnit} onChange={this.handleOnChangeErrorCheckingUnit}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>禁用</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="勾选此项后监测器停止监测">
                                    <input id="IsDisabled_SnmpGigabitInterface" type="checkbox" className="input-checkbox"/>禁用监测器
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>任务计划</td>
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList id="taskScheduler_SnmpGigabitInterface" data={taskSchedulerData} value={this.state.taskScheduler} onChange={this.handleOnChangeTaskScheduler} title="从下拉框中选择该监测器执行何种任务计划"/></td>
                          </tr>
                          <tr>
                              <td className="paddingleft10" style={{width:"15%"}}>描述</td>
                              <td colSpan="5" style={{width:"75%"}}><textarea id="description_SnmpGigabitInterface" className="form-control" style={{height:"60px"}}/></td>
                          </tr>
...
// module.exports = CreateMonitorView_SnmpGigabitInterface;
CreateMonitorView_SnmpGigabitInterface.propTypes = {
...
export default connect(mapStateToProps)(CreateMonitorView_SnmpGigabitInterface)
