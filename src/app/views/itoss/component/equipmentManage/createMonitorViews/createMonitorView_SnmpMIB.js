 ...
 * 计数器 模态框  树结构需要递归算法实现，暂未完成 modified by Fenzhou on 2016/06/21.
 */
require('bootstrap');
...
var CreateMonitorView_SnmpMIB = React.createClass({
    mixins: [History],
    getInitialState: function() {
        return {
            ...
            monitorSNMPMIBFileCur:'',
            selectedMonitorSnmpMIBCounter:''
        };
      },

    componentDidMount: function() {
          if(document.getElementById('createMonitorView_SnmpMIB') != null) {
              document.getElementById('createMonitorView_SnmpMIB').style.height = $(window).height() - 110 - 30 + 'px';
          };
          if (this.isMounted()) {
            const { dispatch, monitorsPropertyData, monitorsPropertyEdit, monitor, monitorSnmpMIBFile, selectedMonitorSnmpMIBCounter, selectedNode } = this.props;
            if(monitorsPropertyEdit){
              ...
              if(monitorsPropertyData){
                ...
                for (var i = 0; i < SpecialProperties.length; i++) {
                  ...
                  if(SpecialProperties[i].indexOf("=") > 0){
                    if(temp[0]=="counts"){
                      ...
                      path = temp[1];
                      this.handleOnChangeSnmpMIBFile(path);
                    }
                  }
                };
                $("#OID_SnmpMIB").val(oid); //oid
                $("#Counts_SnmpMIB").val(counts); //counts

                bUpdateInDidMount = true;
                var Title = monitorsPropertyData.Title;
                $("#Title_SnmpMIB").val(Title); //标题
                var IsRunningAlarm = monitorsPropertyData.IsRunningAlarm;
                if (IsRunningAlarm) {
                  //运用到告警
                  // document.getElementById("IsRunningAlarm_SnmpMIB").checked=true;
                  $("#IsRunningAlarm_SnmpMIB").attr("checked", 'true');
                };
                var RefreshTime = monitorsPropertyData.RefreshTime;
                $("#RefreshTime_SnmpMIB").val(RefreshTime); //刷新频率
                ...
                if (IsErrorChecking) {
                  $("#IsErrorChecking_SnmpMIB").attr("checked", 'true'); //监测器错误校验
                }
                var ErrorCheckingTime = monitorsPropertyData.ErrorCheckingTime;
                $("#ErrorCheckingTime_SnmpMIB").val(ErrorCheckingTime); //错误频率
                ...
                if (IsDisabled) {
                  $("#IsDisabled_SnmpMIB").attr("checked", 'true'); //禁用监测器
                };
                ...
                $("#description_SnmpMIB").val(Description); //描述
              };
            }else{
                //增加监测器
                $("#IsRunningAlarm_SnmpMIB").attr("checked",true);
                $("#RefreshTime_SnmpMIB").val("10");
                $("#Title_SnmpMIB").val("");
                $("#IsErrorChecking_SnmpMIB").attr("checked",false);
                $("#ErrorCheckingTime_SnmpMIB").val("0");
                $("#IsDisabled_SnmpMIB").attr("checked",false);
                $("#description_SnmpMIB").val("");
                $("#ContentMatch_SnmpMIB").val("");
                this.handleOnChangeSnmpMIBFile(this.props.monitorSnmpMIBFile[0]);
            }
          }
      },

    shouldComponentUpdate: function(nextProps, nextState) {
      if (nextProps.selectedMonitorSnmpMIBCounter !== this.props.selectedMonitorSnmpMIBCounter) {
          var counts = nextProps.selectedMonitorSnmpMIBCounter ? nextProps.selectedMonitorSnmpMIBCounter : "";
          $('#Counts_SnmpMIB').val(counts);
    ...
    handleOnClickSave:function(){
      ...
      var oid = $.trim($("#OID_SnmpMIB").val());
      var counts = $.trim($("#Counts_SnmpMIB").val());
      var path = this.state.monitorSNMPMIBFileCur;

      if(!counts){
        $.showPublicDialog("系统提示","请填写计数器。");
        return;
      }else{
        specialProperties = "oid=" + oid + ",counts=" + counts + ",path=" + path;
      };
      var Title = $.trim($("#Title_SnmpMIB").val());//标题
      if(!Title){
      ...
      //运用到告警
      if(document.getElementById("IsRunningAlarm_SnmpMIB").checked){
        IsRunningAlarm = true;
      };
      var RefreshTime = $.trim($("#RefreshTime_SnmpMIB").val()); //刷新频率
      if(!RefreshTime){
      ...
      if (document.getElementById("IsErrorChecking_SnmpMIB").checked) {
        IsErrorChecking = true;
      };
      var ErrorCheckingTime = $.trim($("#ErrorCheckingTime_SnmpMIB").val()); //错误频率;
      ...
      if (document.getElementById("IsDisabled_SnmpMIB").checked) {
        IsDisabled = true; //禁用监测器
      };
      var TaskScheduler = this.state.taskScheduler;//任务计划
      var Description = $.trim($("#description_SnmpMIB").val()); //描述
      ...
      if(monitorsPropertyEdit){
        ...
        var propertyData = {
          ...
          TaskScheduler: TaskScheduler,
          Description: Description,
          MonitorPackage: "com.siteview.monitor.SnmpMIBMonitor",
          GroupId: GroupId,
    ...
    handleOnChangeSnmpMIBFile: function(e) {

        const { dispatch, selectedNode } = this.props;
        this.setState({monitorSNMPMIBFileCur: e});
        var EquipmentId = "";
        var FileName = e;
        if(selectedNode){
          EquipmentId = selectedNode.id;
        ...
        dispatch(EquipmentActions.getMonitorSnmpMIBCounter(filter));
    },

    handleOnChangeTaskScheduler: function(e) {
        this.setState({taskScheduler: e});
    },
    handleOnChangeSnmpMIBCounter: function(e) {
      this.setState({selectedMonitorSnmpMIBCounter: e});

    },

    render: function() {
        const { dispatch, monitorErrorAlarmText, monitorWarningAlarmText, monitorGoodAlarmText, monitorErrorConditionsData,
                monitorWarningConditionsData, monitorGoodConditionsData,monitorsPropertyData,monitorsPropertyEdit, monitorSnmpMIBFile, selectedMonitorSnmpMIBCounter } = this.props;
        return (
            <div id="createMonitorView_SnmpMIB" className="overviewDesViewDiv monitoringSettingTab operationButtons">
                      ...
                      <table>
                        <tbody>
                          <tr>
                            ...
                            <td colSpan="5" style={{width:"75%"}}><input id="Title_SnmpMIB" type="text" title="监测器显示的名称"/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>运用到告警</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="监测器生成事件后告警">
                                    <input id="IsRunningAlarm_SnmpMIB" type="checkbox" className="input-checkbox"/>运用到告警
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>刷新频率<span style={{color: "red"}}>*</span></td>
                            <td style={{width:"25%"}}><input id="RefreshTime_SnmpMIB" type="number" title="监测器的监测频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="RefreshUnit_SnmpMIB" data={timeUnitData} defaultValue={timeUnitData[1]} textField='name'/></td>
                          </tr>

                          <tr>
                            <th rowSpan="5" style={{width:"10%"}}>高级选项</th>
                            <td className="paddingleft10" style={{width:"15%"}}>MIB文件</td>
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList id="MIBFile_SnmpMIB" textField='name' data={this.props.monitorSnmpMIBFile} value={this.state.monitorSNMPMIBFileCur} onChange={this.handleOnChangeSnmpMIBFile}/></td>
                            <td className="paddingleft10" style={{width:"15%"}}>OID</td>
                            <td colSpan="2" style={{width:"30%"}}><input id="OID_SnmpMIB" type="text" title="OID"/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>计数器<span style={{color: "red"}}>*</span></td>
                            <td colSpan="4" style={{width:"65%"}}><textarea className="form-control" style={{height:"60px"}} id="Counts_SnmpMIB"/></td>
                            <td style={{width:"10%"}}><button type="button" className="btn btn-default btnGetModel" style={{width:"100%"}} data-toggle="modal" data-target="#monitorSnmpMIBTableModal">...</button></td>
                          </tr>
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="如果监测器检测到错误，立即再次执行监测器校验错误">
                                    <input id="IsErrorChecking_SnmpMIB" type="checkbox" className="input-checkbox"/>监测器错误校验
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>错误频率</td>
                            <td style={{width:"25%"}}><input id="ErrorCheckingTime_SnmpMIB" type="number" title="处于“错误”条件时监测器刷新频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="ErrorCheckingUnit_SnmpMIB" data={timeUnitData} defaultValue={timeUnitData[1]} textField='name' value={this.state.errorCheckingUnit} onChange={this.handleOnChangeErrorCheckingUnit}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>禁用</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="勾选此项后监测器停止监测">
                                    <input id="IsDisabled_SnmpMIB" type="checkbox" className="input-checkbox"/>禁用监测器
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>任务计划</td>
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList id="taskScheduler_SnmpMIB" data={taskSchedulerData} defaultValue={taskSchedulerData[0]} textField='name' value={this.state.taskScheduler} onChange={this.handleOnChangeTaskScheduler} title="从下拉框中选择监测器执行何种任务计划"/></td>
                          </tr>
                          <tr>
                              <td className="paddingleft10" style={{width:"15%"}}>描述</td>
                              <td colSpan="5" style={{width:"75%"}}><textarea id="description_SnmpMIB" className="form-control" style={{height:"60px"}}/></td>
                          </tr>
...
// module.exports = CreateMonitorView_SnmpMIB;
CreateMonitorView_SnmpMIB.propTypes = {
...
function mapStateToProps(state) {
  const { monitorErrorAlarmText, monitorWarningAlarmText, monitorGoodAlarmText, monitorErrorConditionsData, monitorWarningConditionsData,
          monitorGoodConditionsData, equipmentServerAddress, monitorsPropertyData, monitorsPropertyEdit, selectedNode, monitorSnmpMIBFile, selectedMonitorSnmpMIBCounter } = state.equipmentReducer
...
export default connect(mapStateToProps)(CreateMonitorView_SnmpMIB)
