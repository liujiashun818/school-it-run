...
var CreateMonitorView_AcOnlineusers = React.createClass({
    mixins: [History],
    ...
    componentDidMount: function() {
        if(document.getElementById('createMonitorView_AcOnlineusers') != null) {
            document.getElementById('createMonitorView_AcOnlineusers').style.height = $(window).height() - 110 - 30 + 'px';
        }
        if (this.isMounted()) {
          ...
          if(monitorsPropertyEdit){
            ...
            if(monitorsPropertyData){
              ...
              $("#Title_AcOnlineusers").val(Title); //标题
              var IsRunningAlarm = monitorsPropertyData.IsRunningAlarm;
              if (IsRunningAlarm) {
                //运用到告警
                $("#IsRunningAlarm_AcOnlineusers").attr("checked", 'true');
              };
              var RefreshTime = monitorsPropertyData.RefreshTime;
              $("#RefreshTime_AcOnlineusers").val(RefreshTime); //刷新频率
              ...
              if (IsErrorChecking) {
                $("#IsErrorChecking_AcOnlineusers").attr("checked", 'true'); //监测器错误校验
              }
              var ErrorCheckingTime = monitorsPropertyData.ErrorCheckingTime;
              $("#ErrorCheckingTime_AcOnlineusers").val(ErrorCheckingTime); //错误频率
              ...
              if (IsDisabled) {
                $("#IsDisabled_AcOnlineusers").attr("checked", 'true'); //禁用监测器
              };
              ...
              $("#description_AcOnlineusers").val(Description); //描述
            };
          }else{
              //增加监测器
              $("#IsRunningAlarm_AcOnlineusers").attr("checked",true);
              $("#RefreshTime_AcOnlineusers").val("10");
              $("#Title_AcOnlineusers").val("AC在线用户");
              $("#IsErrorChecking_AcOnlineusers").attr("checked",false);
              $("#ErrorCheckingTime_AcOnlineusers").val("10");
              $("#IsDisabled_AcOnlineusers").attr("checked",false);
              $("#description_AcOnlineusers").val("");
          }
    ...
    handleOnClickSave:function(){
      ...
      //保存监测器数据
      //编辑监测器属性-保存
      var Title = $.trim($("#Title_AcOnlineusers").val());//标题
      if(!Title){
      ...
      //运用到告警
      if(document.getElementById("IsRunningAlarm_AcOnlineusers").checked){
        IsRunningAlarm = true;
      };
      var RefreshTime = $.trim($("#RefreshTime_AcOnlineusers").val()); //刷新频率
      if(!RefreshTime){
      ...
      if (document.getElementById("IsErrorChecking_AcOnlineusers").checked) {
        IsErrorChecking = true;
      };
      var ErrorCheckingTime = $.trim($("#ErrorCheckingTime_AcOnlineusers").val()); //错误频率;
      ...
      if (document.getElementById("IsDisabled_AcOnlineusers").checked) {
        IsDisabled = true; //禁用监测器
      };
      var TaskScheduler = this.state.taskScheduler;//任务计划
      var Description = $.trim($("#description_AcOnlineusers").val()); //描述
      ...
      if(monitorsPropertyEdit){
        ...
        var propertyData = {
          ...
          TaskScheduler: TaskScheduler,
          Description: Description,
          MonitorPackage: "com.siteview.monitor.AcOnlineusersMonitor",
          GroupId: GroupId,
    ...
    render: function() {
      const { dispatch, monitorErrorAlarmText, monitorWarningAlarmText, monitorGoodAlarmText, monitorErrorConditionsData,
        ...
        return (
            <div id="createMonitorView_AcOnlineusers" className="overviewDesViewDiv monitoringSettingTab operationButtons">
                      ...
                      <table>
                        <tbody>
                          <tr>
                            ...
                            <td colSpan="5" style={{width:"75%"}}><input id="Title_AcOnlineusers" type="text"/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>运用到告警</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}}>
                                    <input id="IsRunningAlarm_AcOnlineusers" type="checkbox" className="input-checkbox"/>运用到告警
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>刷新频率<span style={{color: "red"}}>*</span></td>
                            <td style={{width:"25%"}}><input id="RefreshTime_AcOnlineusers" type="number"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="RefreshUnit_AcOnlineusers" data={timeUnitData} value={this.state.refreshUnit} onChange={this.handleOnChangeRefreshUnit}/></td>
                          </tr>
                          ...
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}}>
                                    <input id="IsErrorChecking_AcOnlineusers" type="checkbox" className="input-checkbox"/>监测器错误校验
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>错误频率</td>
                            <td style={{width:"25%"}}><input id="ErrorCheckingTime_AcOnlineusers" type="number"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="ErrorCheckingUnit_AcOnlineusers" data={timeUnitData} value={this.state.errorCheckingUnit} onChange={this.handleOnChangeErrorCheckingUnit}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>禁用</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}}>
                                    <input id="IsDisabled_AcOnlineusers" type="checkbox" className="input-checkbox"/>禁用监测器
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>任务计划</td>
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList id="taskScheduler_AcOnlineusers" data={taskSchedulerData} value={this.state.taskScheduler} onChange={this.handleOnChangeTaskScheduler}/></td>
                          </tr>
                          <tr>
                              <td className="paddingleft10" style={{width:"15%"}}>描述</td>
                              <td colSpan="5" style={{width:"75%"}}><textarea id="description_AcOnlineusers" className="form-control" style={{height:"60px"}}/></td>
                          </tr>
...
// module.exports = CreateMonitorView_AcOnlineusers;
CreateMonitorView_AcOnlineusers.propTypes = {
...
export default connect(mapStateToProps)(CreateMonitorView_AcOnlineusers)
