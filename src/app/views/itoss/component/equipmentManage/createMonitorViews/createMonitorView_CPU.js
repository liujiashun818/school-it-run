...
var CreateMonitorView_CPU = React.createClass({
    mixins: [History],
    ...
    componentDidMount: function() {
        if(document.getElementById('createMonitorView_CPU') != null) {
            document.getElementById('createMonitorView_CPU').style.height = $(window).height() - 110 - 30 + 'px';
        }
        if (this.isMounted()) {
          ...
          if(monitorsPropertyEdit){
            ...
            if(monitorsPropertyData){
              ...
              if (IsErrorChecking) {
                $("#IsErrorChecking_CPU").attr("checked", 'true'); //监测器错误校验
              }
              var ErrorCheckingTime = monitorsPropertyData.ErrorCheckingTime;
              $("#ErrorCheckingTime_CPU").val(ErrorCheckingTime); //错误频率
              ...
              if (IsDisabled) {
                $("#IsDisabled_CPU").attr("checked", 'true'); //禁用监测器
              };
              ...
              $("#description_CPU").val(Description); //描述
            };
          }else{
              //增加监测器
              $("#IsRunningAlarm_CPU").attr("checked",true);
              $("#RefreshTime_CPU").val("10");
              $("#Title_CPU").val("CPU");
              $("#IsErrorChecking_CPU").attr("checked",false);
              $("#ErrorCheckingTime_CPU").val("0");
              $("#IsDisabled_CPU").attr("checked",false);
              $("#description_CPU").val("");
          }
    ...
    handleOnClickSave:function(){
      ...
      //保存监测器数据
      //编辑监测器属性-保存
      var Title = $.trim($("#Title_CPU").val());//标题
      if(!Title){
      ...
      //运用到告警
      if(document.getElementById("IsRunningAlarm_CPU").checked){
        IsRunningAlarm = true;
      };
      var RefreshTime = $.trim($("#RefreshTime_CPU").val()); //刷新频率
      if(!RefreshTime){
      ...
      if (document.getElementById("IsErrorChecking_CPU").checked) {
        IsErrorChecking = true;
      };
      var ErrorCheckingTime = $.trim($("#ErrorCheckingTime_CPU").val()); //错误频率;
      ...
      if (document.getElementById("IsDisabled_CPU").checked) {
        IsDisabled = true; //禁用监测器
      };
      var TaskScheduler = this.state.taskScheduler;//任务计划
      var Description = $.trim($("#description_CPU").val()); //描述
      ...
      if(monitorsPropertyEdit){
        ...
        var propertyData = {
          ...
          TaskScheduler: TaskScheduler,
          Description: Description,
          MonitorPackage: "com.siteview.monitor.CPUMonitor",
          GroupId: GroupId,
    ...
    render: function() {
        ...
        return (
            <div id="createMonitorView_CPU" className="overviewDesViewDiv monitoringSettingTab operationButtons">
                          ...
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="监测器生成事件后告警">
                                    <input id="IsRunningAlarm_CPU" type="checkbox" className="input-checkbox"/>运用到告警
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>刷新频率<span style={{color: "red"}}>*</span></td>
                            <td style={{width:"25%"}}><input id="RefreshTime_CPU" type="number" title="监测器的监测频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="RefreshUnit_CPU" data={timeUnitData} value={this.state.refreshUnit} onChange={this.handleOnChangeRefreshUnit}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>标题<span style={{color: "red"}}>*</span></td>
                            <td colSpan="5" style={{width:"75%"}}><input id="Title_CPU" type="text" title="监测器显示的名称"/></td>
                          </tr>
                          ...
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="如果监测器检测到错误，立即再次执行监测器校验错误">
                                    <input id="IsErrorChecking_CPU" type="checkbox" className="input-checkbox"/>监测器错误校验
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>错误频率</td>
                            <td style={{width:"25%"}}><input id="ErrorCheckingTime_CPU" type="number" title="处于“错误”条件时监测器刷新频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="ErrorCheckingUnit_CPU" data={timeUnitData} value={this.state.errorCheckingUnit} onChange={this.handleOnChangeErrorCheckingUnit}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>禁用</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="勾选此项后监测器停止监测">
                                    <input id="IsDisabled_CPU" type="checkbox" className="input-checkbox"/>禁用监测器
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>任务计划</td>
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList id="taskScheduler_CPU" data={taskSchedulerData} value={this.state.taskScheduler} onChange={this.handleOnChangeTaskScheduler} title="从下拉框中选择监测器执行何种任务计划"/></td>
                          </tr>
                          <tr>
                              <td className="paddingleft10" style={{width:"15%"}}>描述</td>
                              <td colSpan="5" style={{width:"75%"}}><textarea id="description_CPU" className="form-control" style={{height:"60px"}}/></td>
                          </tr>
...
// module.exports = CreateMonitorView_CPU;
CreateMonitorView_CPU.propTypes = {
...
export default connect(mapStateToProps)(CreateMonitorView_CPU)
