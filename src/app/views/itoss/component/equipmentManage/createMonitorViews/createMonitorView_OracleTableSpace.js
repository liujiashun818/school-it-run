...
var CreateMonitorView_OracleTableSpace = React.createClass({
    mixins: [History],
    ...
    componentDidMount: function() {
        if(document.getElementById('createMonitorView_OracleTableSpace') != null) {
            document.getElementById('createMonitorView_OracleTableSpace').style.height = $(window).height() - 110 - 30 + 'px';
        }
        if (this.isMounted()) {
          ...
          if(monitorsPropertyEdit){
            ...
            if(monitorsPropertyData){
              ...
              if (IsErrorChecking) {
                $("#IsErrorChecking_OracleTableSpace").attr("checked", 'true'); //监测器错误校验
              }
              var ErrorCheckingTime = monitorsPropertyData.ErrorCheckingTime;
              $("#ErrorCheckingTime_OracleTableSpace").val(ErrorCheckingTime); //错误频率
              ...
              if (IsDisabled) {
                $("#IsDisabled_OracleTableSpace").attr("checked", 'true'); //禁用监测器
              };
              ...
              $("#description_OracleTableSpace").val(Description); //描述
            };
          }else{
              ...
              //增加监测器
              $("#Title_OracleTableSpace").val("Oracle表空间");
              $("#IsRunningAlarm_OracleTableSpace").attr("checked",true);
              $("#RefreshTime_OracleTableSpace").val("10");
              $("#IsErrorChecking_OracleTableSpace").attr("checked",false);
              $("#ErrorCheckingTime_OracleTableSpace").val("10");
              $("#IsDisabled_OracleTableSpace").attr("checked",false);
              $("#description_OracleTableSpace").val("");
          }
        };
    },
    handleOnClickSave:function(){
      ...
      //保存监测器数据
      //编辑监测器属性-保存
      var Title = $.trim($("#Title_OracleTableSpace").val());//标题
      if(!Title){
      ...
      //运用到告警
      if(document.getElementById("IsRunningAlarm_OracleTableSpace").checked){
        IsRunningAlarm = true;
      };
      var RefreshTime = $.trim($("#RefreshTime_OracleTableSpace").val()); //刷新频率
      if(!RefreshTime){
      ...
      if (document.getElementById("IsErrorChecking_OracleTableSpace").checked) {
        IsErrorChecking = true;
      };
      var ErrorCheckingTime = $.trim($("#ErrorCheckingTime_OracleTableSpace").val()); //错误频率;
      ...
      if (document.getElementById("IsDisabled_OracleTableSpace").checked) {
        IsDisabled = true; //禁用监测器
      };
      var TaskScheduler = this.state.taskScheduler;//任务计划
      var Description = $.trim($("#description_OracleTableSpace").val()); //描述
      ...
      if(monitorsPropertyEdit){
        ...
        var propertyData = {
          ...
          TaskScheduler: TaskScheduler,
          Description: Description,
          MonitorPackage: "com.siteview.monitor.oracletablespace.OracleTableSpaceUi",
          GroupId: GroupId,
    ...
    render: function() {
        ...
        return (
            <div id="createMonitorView_OracleTableSpace" className="overviewDesViewDiv monitoringSettingTab operationButtons">
                      ...
                      <table>
                        <tbody>
                          <tr>
                            ...
                            <td className="paddingleft10" style={{width:"15%"}}>Oracle表空间<span style={{color: "red"}}>*</span></td>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="监测器生成事件后告警">
                                    <input id="IsRunningAlarm_OracleTableSpace" type="checkbox" className="input-checkbox"/>运用到告警
                                </label>
                            </td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>刷新频率<span style={{color: "red"}}>*</span></td>
                            <td style={{width:"25%"}}><input id="RefreshTime_OracleTableSpace" type="number" title="监测器的监测频率"/></td>
                            ...
                            <td colSpan="2" style={{width:"30%"}}><input id="Title_OracleTableSpace" type="text" title="监测器显示的名称"/></td>
                          </tr>
                          ...
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="如果监测器检测到错误，立即再次执行监测器校验错误">
                                    <input id="IsErrorChecking_OracleTableSpace" type="checkbox" className="input-checkbox"/>监测器错误校验
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>错误频率</td>
                            <td style={{width:"25%"}}><input id="ErrorCheckingTime_OracleTableSpace" type="number" title="处于“错误”条件时监测器刷新频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList data={timeUnitData} defaultValue={timeUnitData[1]} textField='name'/></td>
                          </tr>
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="勾选此项后监测器停止监测">
                                    <input id="IsDisabled_OracleTableSpace" type="checkbox" className="input-checkbox"/>禁用监测器
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>任务计划</td>
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList id="taskScheduler_OracleTableSpace" data={taskSchedulerData} defaultValue={taskSchedulerData[0]} textField='name'/></td>
                          </tr>
                          <tr>
                              <td className="paddingleft10" style={{width:"15%"}}>描述</td>
                              <td colSpan="5" style={{width:"75%"}}><textarea id="description_OracleTableSpace" className="form-control" style={{height:"60px"}}/></td>
                          </tr>
...
// module.exports = CreateMonitorView_OracleTableSpace;
CreateMonitorView_OracleTableSpace.propTypes = {
...
export default connect(mapStateToProps)(CreateMonitorView_OracleTableSpace)
