...
var CreateMonitorView_ACPortStatus = React.createClass({
    mixins: [History],
    ...
    componentDidMount: function() {
        if(document.getElementById('createMonitorView_ACPortStatus') != null) {
            document.getElementById('createMonitorView_ACPortStatus').style.height = $(window).height() - 110 - 30 + 'px';
        }
        if (this.isMounted()) {
          ...
          if(monitorsPropertyEdit){
            ...
            if(monitorsPropertyData){
              ...
              var SpecialProperties = SpecialPropertiesTemp.split(","); //ping 的自有属性
              for (var i = 0; i < SpecialProperties.length; i++) {
                if (SpecialProperties[i].indexOf("port") >= 0) {
                  var temp = SpecialProperties[i].split("=");
                  if (temp.length == 2) {
                    if (temp[1]) {
                      $("#Port_ACPortStatus").val(temp[1]);
                    }
                  }
                  break;
                }
              ...
              $("#Title_ACPortStatus").val(Title); //标题
              var IsRunningAlarm = monitorsPropertyData.IsRunningAlarm;
              if (IsRunningAlarm) {
                //运用到告警
                $("#IsRunningAlarm_ACPortStatus").attr("checked", 'true');
              };
              var RefreshTime = monitorsPropertyData.RefreshTime;
              $("#RefreshTime_ACPortStatus").val(RefreshTime); //刷新频率
              ...
              if (IsErrorChecking) {
                $("#IsErrorChecking_ACPortStatus").attr("checked", 'true'); //监测器错误校验
              }
              var ErrorCheckingTime = monitorsPropertyData.ErrorCheckingTime;
              $("#ErrorCheckingTime_ACPortStatus").val(ErrorCheckingTime); //错误频率
              ...
              if (IsDisabled) {
                $("#IsDisabled_ACPortStatus").attr("checked", 'true'); //禁用监测器
              };
              ...
              $("#description_ACPortStatus").val(Description); //描述
            };
          }else{
              //增加监测器
              $("#IsRunningAlarm_ACPortStatus").attr("checked",true);
              $("#RefreshTime_ACPortStatus").val("10");
              $("#Title_ACPortStatus").val("AC端口状态");
              $("#IsErrorChecking_ACPortStatus").attr("checked",false);
              $("#ErrorCheckingTime_ACPortStatus").val("10");
              $("#IsDisabled_ACPortStatus").attr("checked",false);
              $("#description_ACPortStatus").val("");
          }
    ...
    handleOnClickSave:function(){
      ...
      var port = $.trim($("#Port_ACPortStatus").val());
      if(!port){
        $.showPublicDialog("系统提示","请选择端口。");
        return;
      }else{
        specialProperties = "port=" + port;
      };
      var Title = $.trim($("#Title_ACPortStatus").val());//标题
      if(!Title){
      ...
      //运用到告警
      if(document.getElementById("IsRunningAlarm_ACPortStatus").checked){
        IsRunningAlarm = true;
      };
      var RefreshTime = $.trim($("#RefreshTime_ACPortStatus").val()); //刷新频率
      if(!RefreshTime){
      ...
      if (document.getElementById("IsErrorChecking_ACPortStatus").checked) {
        IsErrorChecking = true;
      };
      var ErrorCheckingTime = $.trim($("#ErrorCheckingTime_ACPortStatus").val()); //错误频率;
      ...
      if (document.getElementById("IsDisabled_ACPortStatus").checked) {
        IsDisabled = true; //禁用监测器
      };
      var TaskScheduler = this.state.taskScheduler;//任务计划
      var Description = $.trim($("#description_ACPortStatus").val()); //描述
      ...
      if(monitorsPropertyEdit){
        ...
        var propertyData = {
          ...
          TaskScheduler: TaskScheduler,
          Description: Description,
          MonitorPackage: "com.siteview.monitor.ACPortStatusMonitor",
          GroupId: GroupId,
    ...
    render: function() {
      const { dispatch, monitorErrorAlarmText, monitorWarningAlarmText, monitorGoodAlarmText, monitorErrorConditionsData,
        ...
        return (
            <div id="createMonitorView_ACPortStatus" className="overviewDesViewDiv monitoringSettingTab operationButtons">
                      ...
                      <table>
                        <tbody>
                          <tr>
                            ...
                            <td className="paddingleft10" style={{width:"15%"}}>选择端口<span style={{color: "red"}}>*</span></td>
                            <td style={{width:"25%"}}><input id="Port_ACPortStatus" type="text"/></td>
                            ...
                            <td colSpan="2" style={{width:"30%"}}><input id="Title_ACPortStatus" type="text"/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>运用到告警</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}}>
                                    <input id="IsRunningAlarm_ACPortStatus" type="checkbox" className="input-checkbox"/>运用到告警
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>刷新频率<span style={{color: "red"}}>*</span></td>
                            <td style={{width:"25%"}}><input id="RefreshTime_ACPortStatus" type="number"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="RefreshUnit_ACPortStatus" data={timeUnitData} value={this.state.refreshUnit} onChange={this.handleOnChangeRefreshUnit}/></td>
                          </tr>
                          ...
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}}>
                                    <input id="IsErrorChecking_ACPortStatus" type="checkbox" className="input-checkbox"/>监测器错误校验
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>错误频率</td>
                            <td style={{width:"25%"}}><input id="ErrorCheckingTime_ACPortStatus" type="number"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="ErrorCheckingUnit_ACPortStatus" data={timeUnitData} value={this.state.errorCheckingUnit} onChange={this.handleOnChangeErrorCheckingUnit}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>禁用</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}}>
                                    <input id="IsDisabled_ACPortStatus" type="checkbox" className="input-checkbox"/>禁用监测器
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>任务计划</td>
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList id="taskScheduler_ACPortStatus" data={taskSchedulerData} value={this.state.taskScheduler} onChange={this.handleOnChangeTaskScheduler}/></td>
                          </tr>
                          <tr>
                              <td className="paddingleft10" style={{width:"15%"}}>描述</td>
                              <td colSpan="5" style={{width:"75%"}}><textarea id="description_ACPortStatus" className="form-control" style={{height:"60px"}}/></td>
                          </tr>
...
// module.exports = CreateMonitorView_ACPortStatus;
CreateMonitorView_ACPortStatus.propTypes = {
...
export default connect(mapStateToProps)(CreateMonitorView_ACPortStatus)
