...
var checkContentData = [
  {id:'NO_CHECK_CONTENT_CHANGES', name:'():不检查内容'},
  {id:'SAVE_CHECK_CONTENT_CHANGES', name:'(baseline):与保存的内容比较'},
  {id:'LAST_CHECK_CONTENT_CHANGES', name:'(on):与上次内容比较'},
  {id:'RESET_CHECK_CONTENT_CHANGES', name:'(reset):重置已保存的内容'}
];

var CreateMonitorView_File = React.createClass({
    mixins: [History],
    getInitialState: function() {
        return {
            ...
            checkContent: checkContentData[0]
        }
    },

    componentDidMount: function() {
        if(document.getElementById('createMonitorView_File') != null) {
            document.getElementById('createMonitorView_File').style.height = $(window).height() - 110 - 30 + 'px';
        }
        if (this.isMounted()) {
          ...
          if(monitorsPropertyEdit){
            ...
            if(monitorsPropertyData){
              ...
              for (var i = 0; i < SpecialProperties.length; i++) {
                if (SpecialProperties[i].indexOf("checkContent") >= 0) {
                  var temp = SpecialProperties[i].split("=");
                  if (temp.length == 2) {
                    if (temp[1]) {
                        for(var i = 0; i < checkContentData.length; i++) {
                            if(temp[1] == checkContentData[i].id) {
                                this.setState({checkContent: checkContentData[i]});
                                break;
                            }
                        }
                    }
                  }
                } else if (SpecialProperties[i].indexOf("filePath") >= 0) {
                  var temp = SpecialProperties[i].split("=");
                  if (temp.length == 2) {
                    if (temp[1]) {
                      $("#filePath_File").val(temp[1]);
                    }
                  }
              ...
              if (IsErrorChecking) {
                $("#IsErrorChecking_File").attr("checked", 'true'); //监测器错误校验
              }
              var ErrorCheckingTime = monitorsPropertyData.ErrorCheckingTime;
              $("#ErrorCheckingTime_File").val(ErrorCheckingTime); //错误频率
              ...
              if (IsDisabled) {
                $("#IsDisabled_File").attr("checked", 'true'); //禁用监测器
              };
              ...
              $("#description_File").val(Description); //描述
            };
          }else{
              //增加监测器
              $("#filePath_File").val("");
              $("#IsRunningAlarm_File").attr("checked",true);
              $("#RefreshTime_File").val("10");
              $("#Title_File").val("文件");
              $("#matchContent_File").val("");
              $("#IsErrorChecking_File").attr("checked",false);
              $("#ErrorCheckingTime_File").val("10");
              $("#IsDisabled_File").attr("checked",false);
              $("#description_File").val("");
          }
    ...
    handleOnClickSave:function(){
      ...
      var filePath = $.trim($("#filePath_File").val());
      var matchContent = $.trim($("#matchContent_File").val());
      if(filePath){
        specialProperties = "filePath=" + filePath;
      };
      if(matchContent) {
      ...
      if(specialProperties){
        specialProperties = specialProperties + ",checkContent=" + this.state.checkContent.id;
      }else{
        specialProperties =  "checkContent="+ this.state.checkContent.id;
      }
      var Title = $.trim($("#Title_File").val());//标题
      if(!Title){
      ...
      //运用到告警
      if(document.getElementById("IsRunningAlarm_File").checked){
        IsRunningAlarm = true;
      };
      var RefreshTime = $.trim($("#RefreshTime_File").val()); //刷新频率
      if(!RefreshTime){
      ...
      if (document.getElementById("IsErrorChecking_File").checked) {
        IsErrorChecking = true;
      };
      var ErrorCheckingTime = $.trim($("#ErrorCheckingTime_File").val()); //错误频率;
      ...
      if (document.getElementById("IsDisabled_File").checked) {
        IsDisabled = true; //禁用监测器
      };
      var TaskScheduler = this.state.taskScheduler;//任务计划
      var Description = $.trim($("#description_File").val()); //描述
      ...
      if(monitorsPropertyEdit){
        ...
        var propertyData = {
          ...
          TaskScheduler: TaskScheduler,
          Description: Description,
          MonitorPackage: "com.siteview.monitor.FileMonitor",
          GroupId: GroupId,
    ...
    handleOnChangeCheckContent: function(e) {
        this.setState({checkContent: e});
    },

    handleOnChangeFilePath:function(e){
      var filePath = $.trim($("#filePath_File").val()); //主机
      var Title = "";
      //Title = $.trim($("#Title_File").val());//标题
      Title = "文件:"+ filePath;
      $("#Title_File").val(Title);
    },
    ...
    render: function() {
        ...
        return (
            <div id="createMonitorView_File" className="overviewDesViewDiv monitoringSettingTab operationButtons">
                      ...
                      <table>
                        <tbody>
                          <tr>
                            ...
                            <td className="paddingleft10" style={{width:"15%"}}>文件路径<span style={{color: "red"}}>*</span></td>
                            <td colSpan="2" style={{width:"30%"}}><input id="filePath_File" type="text" onChange={this.handleOnChangeFilePath} title="Unix系统如：/root/scripts/script1，Windows系统如：D:\2.txt，Windows系统共享路径如：//IP/共享文件夹名称/文件名称"/></td>
                            <td className="paddingleft10" style={{width:"15%"}}>运用到告警</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="监测器生成事件后告警">
                                    <input id="IsRunningAlarm_File" type="checkbox" className="input-checkbox"/>运用到告警
                                </label>
                            </td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>刷新频率<span style={{color: "red"}}>*</span></td>
                            <td style={{width:"25%"}}><input id="RefreshTime_File" type="number" title="监测器的监测频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="RefreshUnit_File" data={timeUnitData} value={this.state.refreshUnit} onChange={this.handleOnChangeRefreshUnit}/></td>
                            <td className="paddingleft10" style={{width:"15%"}}>标题<span style={{color: "red"}}>*</span></td>
                            <td colSpan="2" style={{width:"30%"}}><input id="Title_File" type="text" title="监测器显示的名称"/></td>
                          </tr>

                          <tr>
                            <th rowSpan="4" style={{width:"10%"}}>高级选项</th>
                            <td className="paddingleft10" style={{width:"15%"}}>内容匹配</td>
                            <td colSpan="2" style={{width:"30%"}}><input id="matchContent_File" type="text" placeholder="（Windows系统下需要共享文件夹才能对文件夹下的文件内容进行匹配）" title="文本中所包含要匹配的文本字符串"/></td>
                            <td className="paddingleft10" style={{width:"15%"}}>检查内容更改</td>
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList id="checkContent_File" data={checkContentData} value={this.state.checkContent} textField="name" onChange={this.handleOnChangeCheckContent}/></td>
                          </tr>
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="如果监测器检测到错误，立即再次执行监测器校验错误">
                                    <input id="IsErrorChecking_File" type="checkbox" className="input-checkbox"/>监测器错误校验
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>错误频率</td>
                            <td style={{width:"25%"}}><input id="ErrorCheckingTime_File" type="number" title="处于“错误”条件时监测器刷新频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="ErrorCheckingUnit_File" data={timeUnitData} value={this.state.errorCheckingUnit} onChange={this.handleOnChangeErrorCheckingUnit}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>禁用</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="勾选此项后监测器停止监测">
                                    <input id="IsDisabled_File" type="checkbox" className="input-checkbox"/>禁用监测器
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>任务计划</td>
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList id="taskScheduler_File" data={taskSchedulerData} value={this.state.taskScheduler} onChange={this.handleOnChangeTaskScheduler} title="从下拉框中选择监测器执行何种任务计划"/></td>
                          </tr>
                          <tr>
                              <td className="paddingleft10" style={{width:"15%"}}>描述</td>
                              <td colSpan="5" style={{width:"75%"}}><textarea id="description_File" className="form-control" style={{height:"60px"}}/></td>
                          </tr>
...
// module.exports = CreateMonitorView_File;
CreateMonitorView_File.propTypes = {
...
export default connect(mapStateToProps)(CreateMonitorView_File)
