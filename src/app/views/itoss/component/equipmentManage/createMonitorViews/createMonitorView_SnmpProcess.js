...
var CreateMonitorView_SnmpProcess = React.createClass({
    mixins: [History],
    ...
    componentDidMount: function() {
        if(document.getElementById('createMonitorView_SnmpProcess') != null) {
            document.getElementById('createMonitorView_SnmpProcess').style.height = $(window).height() - 110 - 30 + 'px';
        }
        if (this.isMounted()) {
            ...
            if(monitorsPropertyData){
              ...
              for (var i = 0; i < SpecialProperties.length; i++) {
                if (SpecialProperties[i].indexOf("processName") >= 0) {
                  ...
                  if (temp.length == 2) {
                    if (temp[1]) {
                    //   $("#processName_SnmpProcess").val(temp[1]);
                      processName = temp[1];
                    }
                  }
              ...
              dispatch(EquipmentActions.setSelectedMonitorProcess({name:processName, oid:oid}));
              bUpdateInDidMount = true;
              var Title = monitorsPropertyData.Title;
              $("#Title_SnmpProcess").val(Title); //标题
              var IsRunningAlarm = monitorsPropertyData.IsRunningAlarm;
              if (IsRunningAlarm) {
                //运用到告警
                $("#IsRunningAlarm_SnmpProcess").attr("checked", 'true');
              };
              var RefreshTime = monitorsPropertyData.RefreshTime;
              $("#RefreshTime_SnmpProcess").val(RefreshTime); //刷新频率
              ...
              if (IsErrorChecking) {
                $("#IsErrorChecking_SnmpProcess").attr("checked", 'true'); //监测器错误校验
              }
              var ErrorCheckingTime = monitorsPropertyData.ErrorCheckingTime;
              $("#ErrorCheckingTime_SnmpProcess").val(ErrorCheckingTime); //错误频率
              ...
              if (IsDisabled) {
                $("#IsDisabled_SnmpProcess").attr("checked", 'true'); //禁用监测器
              };
              ...
              $("#description_SnmpProcess").val(Description); //描述
            };
          }else{
              //增加监测器
              $("#processName_SnmpProcess").val("");
              $("#IsRunningAlarm_SnmpProcess").attr("checked",true);
              $("#RefreshTime_SnmpProcess").val("10");
              $("#Title_SnmpProcess").val("进程_SNMP");
              $("#IsErrorChecking_SnmpProcess").attr("checked",false);
              $("#ErrorCheckingTime_SnmpProcess").val("10");
              $("#IsDisabled_SnmpProcess").attr("checked",false);
              $("#description_SnmpProcess").val("");
          }
    ...
    shouldComponentUpdate: function(nextProps, nextState) {
        if (!bUpdateInDidMount && nextProps.selectedMonitorProcess !== this.props.selectedMonitorProcess) {
            var processName = nextProps.selectedMonitorProcess ? nextProps.selectedMonitorProcess.name : ""
            var Title = "进程:"+ processName;
            $("#Title_SnmpProcess").val(Title);
        }
    ...
    handleOnClickSave:function(){
      const { dispatch, monitorsPropertyData, monitorsPropertyEdit, selectedNode, selectedMonitorProcess } = this.props;
      //保存监测器数据
      //编辑监测器属性-保存
      var specialProperties = "";
      var processName = $.trim($("#processName_SnmpProcess").val());
      if(!processName){
        $.showPublicDialog("系统提示","请选择进程。");
        return;
      }else{
          if(selectedMonitorProcess.oid == "") {
              specialProperties = "processName=" + processName;
          }
          else {
              specialProperties = "oid=" + selectedMonitorProcess.oid + ",processName=" + processName;
          }
      };
      var Title = $.trim($("#Title_SnmpProcess").val());//标题
      if(!Title){
      ...
      //运用到告警
      if(document.getElementById("IsRunningAlarm_SnmpProcess").checked){
        IsRunningAlarm = true;
      };
      var RefreshTime = $.trim($("#RefreshTime_SnmpProcess").val()); //刷新频率
      if(!RefreshTime){
      ...
      if (document.getElementById("IsErrorChecking_SnmpProcess").checked) {
        IsErrorChecking = true;
      };
      var ErrorCheckingTime = $.trim($("#ErrorCheckingTime_SnmpProcess").val()); //错误频率;
      ...
      if (document.getElementById("IsDisabled_SnmpProcess").checked) {
        IsDisabled = true; //禁用监测器
      };
      var TaskScheduler = this.state.taskScheduler;//任务计划
      var Description = $.trim($("#description_SnmpProcess").val()); //描述
      ...
      if(monitorsPropertyEdit){
        ...
        var propertyData = {
          ...
          TaskScheduler: TaskScheduler,
          Description: Description,
          MonitorPackage: "com.siteview.monitor.SnmpProcessMonitor",
          GroupId: GroupId,
    ...
    handleOnClickGetMonitorProcess:function(){
        ...
        dispatch(EquipmentActions.getMonitorProcess(selectedNode.id, selectedNode.type));
    },
    ...
    render: function() {
        ...
        return (
            <div id="createMonitorView_SnmpProcess" className="overviewDesViewDiv monitoringSettingTab operationButtons">
                      ...
                      <table>
                        <tbody>
                          <tr>
                            ...
                            <td style={{width:"25%"}}><input id="processName_SnmpProcess" type="text" value={selectedMonitorProcess?selectedMonitorProcess.name:""}/></td>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="监测器生成事件后告警">
                                    <input id="IsRunningAlarm_SnmpProcess" type="checkbox" className="input-checkbox"/>运用到告警
                                </label>
                            </td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>刷新频率<span style={{color: "red"}}>*</span></td>
                            <td style={{width:"25%"}}><input id="RefreshTime_SnmpProcess" type="number" title="监测器的监测频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="RefreshUnit_SnmpProcess" data={timeUnitData} value={this.state.refreshUnit} onChange={this.handleOnChangeRefreshUnit}/></td>
                            <td className="paddingleft10" style={{width:"15%"}}>标题<span style={{color: "red"}}>*</span></td>
                            <td colSpan="2" style={{width:"30%"}}><input id="Title_SnmpProcess" type="text" title="监测器的显示名称"/></td>
                          </tr>
                          ...
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="如果监测器检测到错误，立即再次执行监测器以校验错误">
                                    <input id="IsErrorChecking_SnmpProcess" type="checkbox" className="input-checkbox"/>监测器错误校验
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>错误频率</td>
                            <td style={{width:"25%"}}><input id="ErrorCheckingTime_SnmpProcess" type="number" title="处于“错误”条件时监测器的刷新频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="ErrorCheckingUnit_SnmpProcess" data={timeUnitData} value={this.state.errorCheckingUnit} onChange={this.handleOnChangeErrorCheckingUnit}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>禁用</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="勾选此项后监测器停止监测">
                                    <input id="IsDisabled_SnmpProcess" type="checkbox" className="input-checkbox"/>禁用监测器
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>任务计划</td>
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList id="taskScheduler_SnmpProcess" data={taskSchedulerData} value={this.state.taskScheduler} onChange={this.handleOnChangeTaskScheduler} title="从下拉框中选择该监测器执行何种任务计划"/></td>
                          </tr>
                          <tr>
                              <td className="paddingleft10" style={{width:"15%"}}>描述</td>
                              <td colSpan="5" style={{width:"75%"}}><textarea id="description_SnmpProcess" className="form-control" style={{height:"60px"}}/></td>
                          </tr>
...
// module.exports = CreateMonitorView_SnmpProcess;
CreateMonitorView_SnmpProcess.propTypes = {
...
export default connect(mapStateToProps)(CreateMonitorView_SnmpProcess)
