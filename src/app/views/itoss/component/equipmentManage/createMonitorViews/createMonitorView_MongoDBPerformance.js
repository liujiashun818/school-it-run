 ...
 * MongoDBPerformance by Fenzhou on 2016/06/27 完成.
 */
require('bootstrap');
...
var CreateMonitorView_MongoDBPerformance = React.createClass({
    mixins: [History],
    ...
    componentDidMount: function() {
          if(document.getElementById('createMonitorView_MongoDBPerformance') != null) {
              document.getElementById('createMonitorView_MongoDBPerformance').style.height = $(window).height() - 110 - 30 + 'px';
          };
          if (this.isMounted()) {
            ...
            if(monitorsPropertyEdit){
              ...
              if(monitorsPropertyData){
                ...
                $("#Title_MongoDBPerformance").val(Title); //标题
                var IsRunningAlarm = monitorsPropertyData.IsRunningAlarm;
                if (IsRunningAlarm) {
                  //运用到告警
                  // document.getElementById("IsRunningAlarm_MongoDBPerformance").checked=true;
                  $("#IsRunningAlarm_MongoDBPerformance").attr("checked", 'true');
                };
                var RefreshTime = monitorsPropertyData.RefreshTime;
                $("#RefreshTime_MongoDBPerformance").val(RefreshTime); //刷新频率
                ...
                if (IsErrorChecking) {
                  $("#IsErrorChecking_MongoDBPerformance").attr("checked", 'true'); //监测器错误校验
                }
                var ErrorCheckingTime = monitorsPropertyData.ErrorCheckingTime;
                $("#ErrorCheckingTime_MongoDBPerformance").val(ErrorCheckingTime); //错误频率
                ...
                if (IsDisabled) {
                  $("#IsDisabled_MongoDBPerformance").attr("checked", 'true'); //禁用监测器
                };
                ...
                $("#description_MongoDBPerformance").val(Description); //描述
              };
            }else{
                //增加监测器
                $("#IsRunningAlarm_MongoDBPerformance").attr("checked",true);
                $("#RefreshTime_MongoDBPerformance").val("10");
                $("#Title_MongoDBPerformance").val("");
                $("#IsErrorChecking_MongoDBPerformance").attr("checked",false);
                $("#ErrorCheckingTime_MongoDBPerformance").val("0");
                $("#IsDisabled_MongoDBPerformance").attr("checked",false);
                $("#description_MongoDBPerformance").val("");
            }
    ...
    handleOnClickSave:function(){
      ...
      //保存监测器数据
      //编辑监测器属性-保存
      var Title = $.trim($("#Title_MongoDBPerformance").val());//标题
      if(!Title){
      ...
      //运用到告警
      if(document.getElementById("IsRunningAlarm_MongoDBPerformance").checked){
        IsRunningAlarm = true;
      };
      var RefreshTime = $.trim($("#RefreshTime_MongoDBPerformance").val()); //刷新频率
      if(!RefreshTime){
      ...
      if (document.getElementById("IsErrorChecking_MongoDBPerformance").checked) {
        IsErrorChecking = true;
      };
      var ErrorCheckingTime = $.trim($("#ErrorCheckingTime_MongoDBPerformance").val()); //错误频率;
      ...
      if (document.getElementById("IsDisabled_MongoDBPerformance").checked) {
        IsDisabled = true; //禁用监测器
      };
      var TaskScheduler = this.state.taskScheduler;//任务计划
      var Description = $.trim($("#description_MongoDBPerformance").val()); //描述
      ...
      if(monitorsPropertyEdit){
        ...
        var propertyData = {
          RecId: recId,
          MonitorType: "MongoDBPerformance",
        ...
        var propertyData = {
          ...
          TaskScheduler: TaskScheduler,
          Description: Description,
          MonitorPackage: "com.siteview.monitor.mongo.performance.MongoPerformanceUI",
          GroupId: GroupId,
    ...
    render: function() {
        ...
        return (
            <div id="createMonitorView_MongoDBPerformance" className="overviewDesViewDiv monitoringSettingTab operationButtons">
                          ...
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="监测器生成事件后告警">
                                    <input id="IsRunningAlarm_MongoDBPerformance" type="checkbox" className="input-checkbox"/>运用到告警
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>刷新频率<span style={{color: "red"}}>*</span></td>
                            <td style={{width:"25%"}}><input id="RefreshTime_MongoDBPerformance" type="number" title="监测器的监测频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="RefreshUnit_MongoDBPerformance" data={timeUnitData} defaultValue={timeUnitData[1]} textField='name' value={this.state.refreshUnit} onChange={this.handleOnChangeRefreshUnit}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>标题<span style={{color: "red"}}>*</span></td>
                            <td colSpan="5" style={{width:"75%"}}><input id="Title_MongoDBPerformance" type="text" title="监测器显示的名称"/></td>
                          </tr>
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="如果监测器检测到错误，立即再次执行监测器校验错误">
                                    <input id="IsErrorChecking_MongoDBPerformance" type="checkbox" className="input-checkbox"/>监测器错误校验
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>错误频率</td>
                            <td style={{width:"25%"}}><input id="ErrorCheckingTime_MongoDBPerformance" type="number" title="处于“错误”条件时监测器刷新频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="ErrorCheckingUnit_MongoDBPerformance" data={timeUnitData} defaultValue={timeUnitData[1]} textField='name' value={this.state.errorCheckingUnit} onChange={this.handleOnChangeErrorCheckingUnit}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>禁用</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="勾选此项后监测器停止监测">
                                    <input id="IsDisabled_MongoDBPerformance" type="checkbox" className="input-checkbox"/>禁用监测器
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>任务计划</td>
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList id="taskScheduler_MongoDBPerformance" data={taskSchedulerData} defaultValue={taskSchedulerData[0]} textField='name' value={this.state.taskScheduler} onChange={this.handleOnChangeTaskScheduler} title="从下拉框中选择监测器执行何种任务计划"/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>描述</td>
                            <td colSpan="5" style={{width:"75%"}}>
                                <textarea id="description_MongoDBPerformance" className="form-control" rows="5"></textarea>
                            </td>
                          </tr>
...
// module.exports = CreateMonitorView_MongoDBPerformance;
CreateMonitorView_MongoDBPerformance.propTypes = {
...
export default connect(mapStateToProps)(CreateMonitorView_MongoDBPerformance)
