...
var CreateMonitorView_SNMPTrap = React.createClass({
    mixins: [History],
    ...
    componentDidMount: function() {
        if(document.getElementById('createMonitorView_SNMPTrap') != null) {
            document.getElementById('createMonitorView_SNMPTrap').style.height = $(window).height() - 110 - 30 + 'px';
        };
        if (this.isMounted()) {
          ...
          if(monitorsPropertyEdit){
            ...
            if(monitorsPropertyData){
              ...
              var SpecialProperties = SpecialPropertiesTemp.split(","); //ping 的自有属性 SpecialProperties:matchPeriod=3DAY,hostName=192.168.9.12;192.168.9.16,matchValue=message%
              for (var i = 0; i < SpecialProperties.length; i++) {
                if (SpecialProperties[i].indexOf("matchPeriod") >= 0) {
                  //匹配范围
                  var temp = SpecialProperties[i].split("=");
                  if (temp.length == 2) {
                    if (temp[1]) {
                      //$("#matchPeriod_SNMPTrap").val(temp[1]);
                  ...
                  if (temp.length == 2) {
                    if (temp[1]) {
                      $("#matchValue_SNMPTrap").val(temp[1]);//匹配内容
                    }
                  }
                  ...
                  if (temp.length == 2) {
                    if (temp[1]) {
                      $("#hostName_SNMPTrap").val(temp[1]); //IP
                    }
                  }
                }
              };
              var Title = monitorsPropertyData.Title;
              $("#Title_SNMPTrap").val(Title); //标题
              var IsRunningAlarm = monitorsPropertyData.IsRunningAlarm;
              if (IsRunningAlarm) {
                //运用到告警
                // document.getElementById("IsRunningAlarm_SNMPTrap").checked=true;
                $("#IsRunningAlarm_SNMPTrap").attr("checked", 'true');
              };
              var RefreshTime = monitorsPropertyData.RefreshTime;
              $("#RefreshTime_SNMPTrap").val(RefreshTime); //刷新频率
              ...
              if (IsErrorChecking) {
                $("#IsErrorChecking_SNMPTrap").attr("checked", 'true'); //监测器错误校验
              };
              var ErrorCheckingTime = monitorsPropertyData.ErrorCheckingTime;
              $("#ErrorCheckingTime_SNMPTrap").val(ErrorCheckingTime); //错误频率
              ...
              if (IsDisabled) {
                $("#IsDisabled_SNMPTrap").attr("checked", 'true'); //禁用监测器
              };
              ...
              $("#description_SNMPTrap").val(Description); //描述
            };
          }else{
              //增加监测器
              $("#Title_SNMPTrap").val("SNMP Trap");//标题
              $("#IsRunningAlarm_SNMPTrap").attr("checked",true);
              $("#RefreshTime_SNMPTrap").val("10");
              $("#IsErrorChecking_SNMPTrap").attr("checked",false);
              $("#ErrorCheckingTime_SNMPTrap").val("10");
              $("#IsDisabled_SNMPTrap").attr("checked",false);
              $("#description_SNMPTrap").val("");
          }
        };
    },
    handleOnClickSave:function(){
      ...
      //保存监测器数据
      //编辑监测器属性-保存
      var specialProperties = ""; //的自有属性 SpecialProperties:matchPeriod=3DAY,hostName=192.168.9.12;192.168.9.16,matchValue=message%
      var matchingRange = this.state.matchingRange;//匹配范围
      var matchingContent = $.trim($("#matchValue_SNMPTrap").val());//匹配内容
      var hostname = $.trim($("#hostName_SNMPTrap").val()); //IP地址
      if(matchingRange){
      ...
      var Title = $.trim($("#Title_SNMPTrap").val());//标题
      if(!Title){
      ...
      //运用到告警
      if(document.getElementById("IsRunningAlarm_SNMPTrap").checked){
        IsRunningAlarm = true;
      };
      var RefreshTime = $.trim($("#RefreshTime_SNMPTrap").val()); //刷新频率
      if(!RefreshTime){
      ...
      if (document.getElementById("IsErrorChecking_SNMPTrap").checked) {
        IsErrorChecking = true;
      };
      var ErrorCheckingTime = $.trim($("#ErrorCheckingTime_SNMPTrap").val()); //错误频率;
      ...
      if (document.getElementById("IsDisabled_SNMPTrap").checked) {
        IsDisabled = true; //禁用监测器
      };
      var TaskScheduler = this.state.taskScheduler;//任务计划
      var Description = $.trim($("#description_SNMPTrap").val()); //描述
      ...
      if(monitorsPropertyEdit){
        ...
        var propertyData = {
          ...
          TaskScheduler: TaskScheduler,
          Description: Description,
          MonitorPackage: "com.siteview.monitor.SNMPTrapMonitor",
          GroupId: GroupId,
    ...
    render: function() {
        ...
        return (
            <div id="createMonitorView_SNMPTrap" className="overviewDesViewDiv monitoringSettingTab operationButtons">
                      ...
                      <table>
                        <tbody>
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}><input id="hostName_SNMPTrap" type="text" title="产生SNMP TRAP的设备IP，如果匹配多个IP，IP之间用；分隔如192.168.5.1;192.168.5.2不输入任何值匹配所有设备"/></td>
                            <td className="paddingleft10" style={{width:"15%"}}>运用到告警</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="监测器生成事件后告警">
                                    <input id="IsRunningAlarm_SNMPTrap" type="checkbox" className="input-checkbox"/>运用到告警
                                </label>
                            </td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>刷新频率<span style={{color: "red"}}>*</span></td>
                            <td style={{width:"25%"}}><input id="RefreshTime_SNMPTrap" type="number" title="监测器的监测频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="RefreshUnit_SNMPTrap"  data={timeUnitData} value={this.state.refreshUnit} onChange={this.handleOnChangeRefreshUnit}/></td>
                            <td className="paddingleft10" style={{width:"15%"}}>标题<span style={{color: "red"}}>*</span></td>
                            <td colSpan="2" style={{width:"30%"}}><input id="Title_SNMPTrap" type="text" title="监测器显示的名称"/></td>
                          </tr>

                          <tr>
                            <th rowSpan="4" style={{width:"10%"}}>高级选项</th>
                            <td className="paddingleft10" style={{width:"15%"}}>匹配范围</td>
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList id="matchPeriod_SNMPTrap" data={matchingRangeData} value={this.state.matchingRange} textField='name' onChange={this.handleOnChangeMatchingRange} title="匹配的时间范围"/></td>
                            <td className="paddingleft10" style={{width:"15%"}}>匹配内容</td>
                            <td colSpan="2" style={{width:"30%"}}><input id="matchValue_SNMPTrap" type="text" title="需要匹配的内容，如：前匹配message%，后匹配%message，全匹配message，包含匹配%message%"/></td>
                          </tr>
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="如果监测器检测到错误，立即再次执行监测器校验错误">
                                    <input id="IsErrorChecking_SNMPTrap" type="checkbox" className="input-checkbox"/>监测器错误校验
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>错误频率</td>
                            <td style={{width:"25%"}}><input id="ErrorCheckingTime_SNMPTrap" type="number" title="处于“错误”条件时监测器刷新频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="ErrorCheckingUnit_SNMPTrap" data={timeUnitData} value={this.state.errorCheckingUnit} onChange={this.handleOnChangeErrorCheckingUnit}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>禁用</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="勾选此项后监测器停止监测">
                                    <input id="IsDisabled_SNMPTrap" type="checkbox" className="input-checkbox"/>禁用监测器
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>任务计划</td>
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList id="taskScheduler_SNMPTrap" data={taskSchedulerData} value={this.state.taskScheduler} onChange={this.handleOnChangeTaskScheduler} title="从下拉框中选择监测器执行何种任务计划"/></td>
                          </tr>
                          <tr>
                              <td className="paddingleft10" style={{width:"15%"}}>描述</td>
                              <td colSpan="5" style={{width:"75%"}}><textarea id="description_SNMPTrap" className="form-control" style={{height:"60px"}}/></td>
                          </tr>
...
// module.exports = CreateMonitorView_SNMPTrap;
CreateMonitorView_SNMPTrap.propTypes = {
...
export default connect(mapStateToProps)(CreateMonitorView_SNMPTrap)
