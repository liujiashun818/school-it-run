/**
 * Created by SHIN on 2016/05/25.
 * webApp名称字段保存没有确定 没有做完,“获取”按钮没有写调取后台接口功能。
 */
require('bootstrap');
...
var CreateMonitorView_WeblogicWebApp = React.createClass({
    mixins: [History],
    ...
    componentDidMount: function() {
        if(document.getElementById('createMonitorView_WeblogicWebApp') != null) {
            document.getElementById('createMonitorView_WeblogicWebApp').style.height = $(window).height() - 110 - 30 + 'px';
        };

        if (this.isMounted()) {
          document.getElementById('hostname_WeblogicWebApp').disabled = true;//webApp名称
          ...
          if(monitorsPropertyEdit){
            ...
            if(monitorsPropertyData){
              ...
              var SpecialProperties = SpecialPropertiesTemp.split(","); //自有属性
              for (var i = 0; i < SpecialProperties.length; i++) {
                if (SpecialProperties[i].indexOf("hostname") >= 0) {
                  //webApp名称
                  var temp = SpecialProperties[i].split("=");
                  if (temp.length == 2) {
                    if (temp[1]) {
                      $("#hostname_WeblogicWebApp").val(temp[1]); //webApp名称
                    }
                  }
                }
              };
              var Title = monitorsPropertyData.Title;
              $("#Title_WeblogicWebApp").val(Title); //标题
              var IsRunningAlarm = monitorsPropertyData.IsRunningAlarm;
              if (IsRunningAlarm) {
                //运用到告警
                // document.getElementById("IsRunningAlarm_WeblogicWebApp").checked=true;
                $("#IsRunningAlarm_WeblogicWebApp").attr("checked", 'true');
              };
              var RefreshTime = monitorsPropertyData.RefreshTime;
              $("#RefreshTime_WeblogicWebApp").val(RefreshTime); //刷新频率
              ...
              if (IsErrorChecking) {
                $("#IsErrorChecking_WeblogicWebApp").attr("checked", 'true'); //监测器错误校验
              };
              var ErrorCheckingTime = monitorsPropertyData.ErrorCheckingTime;
              $("#ErrorCheckingTime_WeblogicWebApp").val(ErrorCheckingTime); //错误频率
              ...
              if (IsDisabled) {
                $("#IsDisabled_WeblogicWebApp").attr("checked", 'true'); //禁用监测器
              };
              ...
              $("#description_WeblogicWebApp").val(Description); //描述
            };
          }else{
              //增加监测器
              $("#IsRunningAlarm_WeblogicWebApp").attr("checked",true);
              $("#RefreshTime_WeblogicWebApp").val("10");
              $("#Title_WeblogicWebApp").val("WeblogicWebApp");
              $("#IsErrorChecking_WeblogicWebApp").attr("checked",false);
              $("#ErrorCheckingTime_WeblogicWebApp").val("0");
              $("#IsDisabled_WeblogicWebApp").attr("checked",false);
              $("#description_WeblogicWebApp").val("");
          }
        };
    },
    ...
    handleOnClickSave:function(){
      ...
      //保存监测器数据
      //编辑监测器属性-保存
      var specialProperties = ""; //的自有属性
      var hostname = $.trim($("#hostname_WeblogicWebApp").val()); //WebApp名称
      if(!hostname){
        $.showPublicDialog("系统提示","请获取WebApp名称。");
        return;
      };
      if(hostname) {
          specialProperties = "hostname=" + hostname;
      };
      var Title = $.trim($("#Title_WeblogicWebApp").val());//标题
      if(!Title){
      ...
      //运用到告警
      if(document.getElementById("IsRunningAlarm_WeblogicWebApp").checked){
        IsRunningAlarm = true;
      };
      var RefreshTime = $.trim($("#RefreshTime_WeblogicWebApp").val()); //刷新频率
      if(!RefreshTime){
      ...
      if (document.getElementById("IsErrorChecking_WeblogicWebApp").checked) {
        IsErrorChecking = true;
      };
      var ErrorCheckingTime = $.trim($("#ErrorCheckingTime_WeblogicWebApp").val()); //错误频率;
      ...
      if (document.getElementById("IsDisabled_WeblogicWebApp").checked) {
        IsDisabled = true; //禁用监测器
      };
      var TaskScheduler = this.state.taskScheduler;//任务计划
      var Description = $.trim($("#description_WeblogicWebApp").val()); //描述
      ...
      if(monitorsPropertyEdit){
        ...
        var propertyData = {
          ...
          TaskScheduler: TaskScheduler,
          Description: Description,
          MonitorPackage: "com.siteview.monitor.WeblogicWebAppMonitor",
          GroupId: GroupId,
    ...
    render: function() {
      const { dispatch, monitorErrorAlarmText, monitorWarningAlarmText, monitorGoodAlarmText, monitorErrorConditionsData,
        ...
        return (
            <div id="createMonitorView_WeblogicWebApp" className="overviewDesViewDiv monitoringSettingTab operationButtons">
                      ...
                      <table>
                        <tbody>
                          <tr>
                            ...
                            <td className="paddingleft10" style={{width:"15%"}}>WebApp名称<span style={{color: "red"}}>*</span></td>
                            <td style={{width:"25%"}}><input id="hostname_WeblogicWebApp" type="text"/></td>
                            <td style={{width:"5%"}}><button type="button" className="btn btn-default btnGetModel" style={{width:"100%"}}>获取</button></td>
                            <td className="paddingleft10" style={{width:"15%"}}>标题<span style={{color: "red"}}>*</span></td>
                            <td colSpan="2" style={{width:"30%"}}><input id="Title_WeblogicWebApp" type="text"/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>运用到告警</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}}>
                                    <input id="IsRunningAlarm_WeblogicWebApp" type="checkbox" className="input-checkbox"/>运用到告警
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>刷新频率<span style={{color: "red"}}>*</span></td>
                            <td style={{width:"25%"}}><input id="RefreshTime_WeblogicWebApp" type="text"/></td>
                            <td style={{width:"5%"}}>
                              <ReactWidgets.DropdownList className="dataDictDropDown" id="RefreshUnit_WeblogicWebApp"  data={timeUnitData} value={this.state.refreshUnit} onChange={this.handleOnChangeRefreshUnit}/>
                            </td>
                          </tr>
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}}>
                                    <input id="IsErrorChecking_WeblogicWebApp" type="checkbox" className="input-checkbox"/>监测器错误校验
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>错误频率</td>
                            <td style={{width:"25%"}}><input id="ErrorCheckingTime_WeblogicWebApp" type="text"/></td>
                            <td style={{width:"5%"}}>
                              <ReactWidgets.DropdownList className="dataDictDropDown" id="ErrorCheckingUnit_WeblogicWebApp" data={timeUnitData} value={this.state.errorCheckingUnit} onChange={this.handleOnChangeErrorCheckingUnit}/>
                            </td>
                          </tr>
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}}>
                                    <input id="IsDisabled_WeblogicWebApp" type="checkbox" className="input-checkbox"/>禁用监测器
                                </label>
                            </td>
                            ...
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList id="taskScheduler_WeblogicWebApp" data={taskSchedulerData} value={this.state.taskScheduler} onChange={this.handleOnChangeTaskScheduler}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>描述</td>
                            <td colSpan="5" style={{width:"75%"}}>
                                <textarea id="description_WeblogicWebApp" className="form-control" rows="5"></textarea>
                            </td>
                          </tr>
...
// module.exports = CreateMonitorView_WeblogicWebApp;
CreateMonitorView_WeblogicWebApp.propTypes = {
...
export default connect(mapStateToProps)(CreateMonitorView_WeblogicWebApp)
