 ...
 * SQLStatic by Fenzhou on 2016/06/21 完成.
 */
require('bootstrap');
...
var CreateMonitorView_SQLStatic = React.createClass({
    mixins: [History],
    ...
    componentDidMount: function() {
        if(document.getElementById('createMonitorView_SQLStatic') != null) {
            document.getElementById('createMonitorView_SQLStatic').style.height = $(window).height() - 110 - 30 + 'px';
        };
        if (this.isMounted()) {
          ...
          if(monitorsPropertyEdit){
            ...
            if(monitorsPropertyData){
              ...
              $("#Title_SQLStatic").val(Title); //标题
              var IsRunningAlarm = monitorsPropertyData.IsRunningAlarm;
              if (IsRunningAlarm) {
                //运用到告警
                // document.getElementById("IsRunningAlarm_SQLStatic").checked=true;
                $("#IsRunningAlarm_SQLStatic").attr("checked", 'true');
              };
              var RefreshTime = monitorsPropertyData.RefreshTime;
              $("#RefreshTime_SQLStatic").val(RefreshTime); //刷新频率
              ...
              if (IsErrorChecking) {
                $("#IsErrorChecking_SQLStatic").attr("checked", 'true'); //监测器错误校验
              }
              var ErrorCheckingTime = monitorsPropertyData.ErrorCheckingTime;
              $("#ErrorCheckingTime_SQLStatic").val(ErrorCheckingTime); //错误频率
              ...
              if (IsDisabled) {
                $("#IsDisabled_SQLStatic").attr("checked", 'true'); //禁用监测器
              };
              ...
              $("#description_SQLStatic").val(Description); //描述
            };
          }else{
              //增加监测器
              $("#IsRunningAlarm_SQLStatic").attr("checked",true);
              $("#RefreshTime_SQLStatic").val("10");
              $("#Title_SQLStatic").val("");
              $("#IsErrorChecking_SQLStatic").attr("checked",false);
              $("#ErrorCheckingTime_SQLStatic").val("0");
              $("#IsDisabled_SQLStatic").attr("checked",false);
              $("#description_SQLStatic").val("");
          }
    ...
    handleOnClickSave:function(){
      ...
      //保存监测器数据
      //编辑监测器属性-保存
      var Title = $.trim($("#Title_SQLStatic").val());//标题
      if(!Title){
      ...
      //运用到告警
      if(document.getElementById("IsRunningAlarm_SQLStatic").checked){
        IsRunningAlarm = true;
      };
      var RefreshTime = $.trim($("#RefreshTime_SQLStatic").val()); //刷新频率
      if(!RefreshTime){
      ...
      if (document.getElementById("IsErrorChecking_SQLStatic").checked) {
        IsErrorChecking = true;
      };
      var ErrorCheckingTime = $.trim($("#ErrorCheckingTime_SQLStatic").val()); //错误频率;
      ...
      if (document.getElementById("IsDisabled_SQLStatic").checked) {
        IsDisabled = true; //禁用监测器
      };
      var TaskScheduler = this.state.taskScheduler;//任务计划
      var Description = $.trim($("#description_SQLStatic").val()); //描述
      ...
      if(monitorsPropertyEdit){
        ...
        var propertyData = {
          ...
          TaskScheduler: TaskScheduler,
          Description: Description,
          MonitorPackage: "com.siteview.monitor.sqlstaticmanager.SQLStaticManagerMonitor",
          GroupId: GroupId,
    ...
    render: function() {
        ...
        return (
            <div id="createMonitorView_SQLStatic" className="overviewDesViewDiv monitoringSettingTab operationButtons">
                          ...
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="监测器生成事件后告警">
                                    <input id="IsRunningAlarm_SQLStatic" type="checkbox" className="input-checkbox"/>运用到告警
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>刷新频率<span style={{color: "red"}}>*</span></td>
                            <td style={{width:"25%"}}><input id="RefreshTime_SQLStatic" type="number" title="监测器的监测频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="RefreshUnit_SQLStatic" data={timeUnitData} value={this.state.refreshUnit} onChange={this.handleOnChangeRefreshUnit}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>标题<span style={{color: "red"}}>*</span></td>
                            <td colSpan="5" style={{width:"75%"}}><input id="Title_SQLStatic" type="text" title="监测器显示的名称"/></td>
                          </tr>
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="如果监测器检测到错误，立即再次执行监测器校验错误">
                                    <input id="IsErrorChecking_SQLStatic" type="checkbox" className="input-checkbox"/>监测器错误校验
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>错误频率</td>
                            <td style={{width:"25%"}}><input id="ErrorCheckingTime_SQLStatic" type="number" title="处于“错误”条件时监测器刷新频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="ErrorCheckingUnit_SQLStatic" data={timeUnitData} value={this.state.errorCheckingUnit} onChange={this.handleOnChangeErrorCheckingUnit}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>禁用</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="勾选此项后监测器停止监测">
                                    <input type="checkbox" id="IsDisabled_SQLStatic" className="input-checkbox"/>禁用监测器
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>任务计划</td>
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList id="taskScheduler_SQLStatic" data={taskSchedulerData} value={this.state.taskScheduler} onChange={this.handleOnChangeTaskScheduler} title="从下拉框中选择监测器执行何种任务计划"/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>描述</td>
                            <td colSpan="5" style={{width:"75%"}}>
                                <textarea id="description_SQLStatic" className="form-control" rows="5"></textarea>
                            </td>
                          </tr>
...
CreateMonitorView_SQLStatic.propTypes = {
...
export default connect(mapStateToProps)(CreateMonitorView_SQLStatic)
