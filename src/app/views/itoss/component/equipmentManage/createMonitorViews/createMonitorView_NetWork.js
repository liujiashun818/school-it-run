 ...
 * NetWork by Fenzhou on 2016/06/15 完成.
 */
require('bootstrap');
...
var CreateMonitorView_NetWork = React.createClass({
    mixins: [History],
    ...
    componentDidMount: function() {
          if(document.getElementById('createMonitorView_NetWork') != null) {
              document.getElementById('createMonitorView_NetWork').style.height = $(window).height() - 110 - 30 + 'px';
          };
          if (this.isMounted()) {
            ...
            if(monitorsPropertyEdit){
              ...
              if(monitorsPropertyData){
                ...
                dispatch(EquipmentActions.setSelectedMonitorNetwork({networkName:networkName})); //网卡
                bUpdateInDidMount = true;
                var Title = monitorsPropertyData.Title;
                $("#Title_NetWork").val(Title); //标题
                var IsRunningAlarm = monitorsPropertyData.IsRunningAlarm;
                if (IsRunningAlarm) {
                  //运用到告警
                  // document.getElementById("IsRunningAlarm_NetWork").checked=true;
                  $("#IsRunningAlarm_NetWork").attr("checked", 'true');
                };
                var RefreshTime = monitorsPropertyData.RefreshTime;
                $("#RefreshTime_NetWork").val(RefreshTime); //刷新频率
                ...
                if (IsErrorChecking) {
                  $("#IsErrorChecking_NetWork").attr("checked", 'true'); //监测器错误校验
                }
                var ErrorCheckingTime = monitorsPropertyData.ErrorCheckingTime;
                $("#ErrorCheckingTime_NetWork").val(ErrorCheckingTime); //错误频率
                ...
                if (IsDisabled) {
                  $("#IsDisabled_NetWork").attr("checked", 'true'); //禁用监测器
                };
                ...
                $("#description_NetWork").val(Description); //描述
              };
            }else{
                //增加监测器
                $("#networkName_NetWork").val("");
                $("#IsRunningAlarm_NetWork").attr("checked",true);
                $("#RefreshTime_NetWork").val("10");
                $("#Title_NetWork").val("");
                $("#IsErrorChecking_NetWork").attr("checked",false);
                $("#ErrorCheckingTime_NetWork").val("0");
                $("#IsDisabled_NetWork").attr("checked",false);
                $("#description_NetWork").val("");
            }
    ...
    //           var Title = "磁盘:"+ networkName;
    //           $("#Title_NetWork").val(Title);
    //       }
    ...
    shouldComponentUpdate: function(nextProps, nextState) {
        if (!bUpdateInDidMount && nextProps.selectedMonitorNetwork !== this.props.selectedMonitorNetwork) {
            var networkName = nextProps.selectedMonitorNetwork ? nextProps.selectedMonitorNetwork.networkName : ""
            var Title = "网卡:"+ networkName;
            $("#Title_NetWork").val(Title);
        }
        bUpdateInDidMount = false;
        return true;
    ...
    handleOnClickSave:function(){
      ...
      var networkName = $.trim($("#networkName_NetWork").val());
      if(!networkName){
        $.showPublicDialog("系统提示","请选择网卡。");
        return;
      }else{
        specialProperties = "networkName=" + networkName; //网卡
      };
      var Title = $.trim($("#Title_NetWork").val());//标题
      if(!Title){
      ...
      //运用到告警
      if(document.getElementById("IsRunningAlarm_NetWork").checked){
        IsRunningAlarm = true;
      };
      var RefreshTime = $.trim($("#RefreshTime_NetWork").val()); //刷新频率
      if(!RefreshTime){
      ...
      if (document.getElementById("IsErrorChecking_NetWork").checked) {
        IsErrorChecking = true;
      };
      var ErrorCheckingTime = $.trim($("#ErrorCheckingTime_NetWork").val()); //错误频率;
      ...
      if (document.getElementById("IsDisabled_NetWork").checked) {
        IsDisabled = true; //禁用监测器
      };
      var TaskScheduler = this.state.taskScheduler;//任务计划
      var Description = $.trim($("#description_NetWork").val()); //描述
      ...
      if(monitorsPropertyEdit){
        ...
        var propertyData = {
          ...
          TaskScheduler: TaskScheduler,
          Description: Description,
          MonitorPackage: "com.siteview.monitor.NetworkMonitor",
          GroupId: GroupId,
    ...
    render: function() {
        ...
        return (
            <div id="createMonitorView_NetWork" className="overviewDesViewDiv monitoringSettingTab operationButtons">
                      ...
                      <table>
                        <tbody>
                          <tr>
                            ...
                            <td style={{width:"25%"}}><input id="networkName_NetWork" type="text" value={selectedMonitorNetwork?selectedMonitorNetwork.networkName:""}/></td>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="监测器生成事件后告警">
                                    <input id="IsRunningAlarm_NetWork" type="checkbox" className="input-checkbox"/>运用到告警
                                </label>
                            </td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>刷新频率<span style={{color: "red"}}>*</span></td>
                            <td style={{width:"25%"}}><input id="RefreshTime_NetWork" type="number" title="监测器的监测频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="RefreshUnit_NetWork" data={timeUnitData} defaultValue={timeUnitData[1]} textField='name'/></td>
                            <td className="paddingleft10" style={{width:"15%"}}>标题<span style={{color: "red"}}>*</span></td>
                            <td colSpan="2" style={{width:"30%"}}><input id="Title_NetWork" type="text" title="监测器显示的名称"/></td>
                          </tr>
                          ...
                          <tr>
                            ...
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="如果监测器检测到错误，立即再次执行监测器校验错误">
                                    <input id="IsErrorChecking_NetWork" type="checkbox" className="input-checkbox"/>监测器错误校验
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>错误频率</td>
                            <td style={{width:"25%"}}><input id="ErrorCheckingTime_NetWork" type="number" title="处于“错误”条件时监测器刷新频率"/></td>
                            <td style={{width:"5%"}}><ReactWidgets.DropdownList id="ErrorCheckingUnit_NetWork" data={timeUnitData} defaultValue={timeUnitData[1]} textField='name' value={this.state.errorCheckingUnit} onChange={this.handleOnChangeErrorCheckingUnit}/></td>
                          </tr>
                          <tr>
                            <td className="paddingleft10" style={{width:"15%"}}>禁用</td>
                            <td colSpan="2" style={{width:"30%"}}>
                                <label style={{marginBottom:"0",paddingLeft:"5px"}} title="勾选此项后监测器停止监测">
                                    <input id="IsDisabled_NetWork" type="checkbox" className="input-checkbox"/>禁用监测器
                                </label>
                            </td>
                            <td className="paddingleft10" style={{width:"15%"}}>任务计划</td>
                            <td colSpan="2" style={{width:"30%"}}><ReactWidgets.DropdownList id="taskScheduler_NetWork" data={taskSchedulerData} defaultValue={taskSchedulerData[0]} textField='name' value={this.state.taskScheduler} onChange={this.handleOnChangeTaskScheduler} title="从下拉框中选择监测器执行何种任务计划"/></td>
                          </tr>
                          <tr>
                              <td className="paddingleft10" style={{width:"15%"}}>描述</td>
                              <td colSpan="5" style={{width:"75%"}}><textarea id="description_NetWork" className="form-control" style={{height:"60px"}}/></td>
                          </tr>
...
// module.exports = CreateMonitorView_NetWork;
CreateMonitorView_NetWork.propTypes = {
...
export default connect(mapStateToProps)(CreateMonitorView_NetWork)
